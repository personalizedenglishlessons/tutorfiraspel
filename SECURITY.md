...
| 18 | Security headers | DONE (meta) + LIMIT (HSTS) | Content-Security-Policy meta tag on ALL 6 HTML pages (app.html, login.html, index.html, admin.html, verify.html, legal.html), each scoped to its actual resource requirements. `frame-ancestors` removed from meta CSP (ignored by browsers in `<meta>` — causes console errors); clickjacking protection relies on `X-Frame-Options: DENY` + `frame-ancestors 'none'` in the `_headers` file for Cloudflare/Netlify migration. HSTS is response-only and needs a host that sets headers. `wasm-unsafe-eval` added to `script-src` in app.html CSP to enable Whisper on-device speech recognition (without it, WASM compilation is blocked and Whisper silently falls back to server-side transcription). `Permissions-Policy: microphone=(self)` in `_headers` allows the pronunciation recording feature when migrating to Cloudflare/Netlify. |
