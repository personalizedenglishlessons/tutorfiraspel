/* ============================================================
   ...
   - Hard caps: 10-minute countdown, max 20 questions.
   - On termination (time / 20 / 3-miss): freeze input, persist result
...
(function () {
  ...
  var HARD_CAP = 20;            // max questions per session
  ...
  var L = {
    ...
    introSub: { en: 'Ready to see your level? Let us jump straight in. 10 minutes, up to 20 questions, and we adapt to you as you go.', ar: 'جاهز تشوف مستواك؟ خلنا نبدا علطول. 10 دقايق، حتى 20 سوال، ونتعشانف معاك وانت جاي.' },
    start: { en: 'Jump straight in', ar: 'نبدا علطول' },
    gate: { en: 'Can you read and pronounce English letters?', ar: 'تيمكنر تقرا وتنطق حروف انجليزي؟' },
    gateSub: { en: 'Be honest. This picks the right test for you.', ar: 'كون صريح. عشان نختار الاختبار المناسب لك.' },
    gateYes: { en: 'Yes, I can', ar: 'اي، ايمكنر' },
    ...
    evaluatingSub: { en: 'Matching you to the right plan', ar: 'نطابقك مع الباقه المناسبه' },
    resultBeginner: { en: 'You are starting from zero. The Start From 0 plan takes you from the very first letter, step by step.', ar: 'مستواك يبدا من الصفر. خطه ابد من الصفر تاخذك من اول حرف خطوه بخطوه.' },
    resultExam: { en: 'You are advanced. The Exam Prep plan fits you (STEP, TOEFL, IELTS).', ar: 'مستواك متيمكنم. خطه التجهيز للاختبارات تناسبك (STEP و TOEFL و IELTS).' },
    ...
    recommended: { en: 'Recommended plan', ar: 'الباقه المناسبه' },
    chooseOther: { en: 'Or pick the other track', ar: 'او اختر المسار الثاني' },
    bestValue: { en: 'Best value', ar: 'افضل قيمه' },
    classesWeek: { en: 'live classes / week', ar: 'حصص مباشره كل اسبوع' },
    classesWeekNote: { en: 'included with this plan', ar: 'مشموله معاك بهالباقه' },
    enroll: { en: 'Enroll via WhatsApp', ar: 'سجل عن طريق واتساب' },
    activatedNote: { en: 'Activation within minutes after manual confirmation.', ar: 'التنشيط خلال دقايق لسه التاعشاند اليدوي.' },
    buyCredits: { en: 'Want extra live classes?', ar: 'تبغى حصص اضافيه؟' },
  ...
  /* ---------------- lifecycle ---------------- */
  function open() {
    if(ST.timerInt){ clearInterval(ST.timerInt); ST.timerInt = null; } // clear stale timer
    ...
    ensureQuestions().then(function (qs) {
      ...
      renderIntro();
    }).catch(function(){ ST.pool = FALLBACK_QUESTIONS; renderIntro(); });
  }
  function close() {
  ...
  /* A learner who cannot read English letters gets a transliteration track:
     ...
     Arabic script (e.g. عشانفك -> هاو ار يو). Tests pronunciation/recognition,
  ...
  function shouldStop() {
    ...
    if (ST.asked.length >= HARD_CAP) return true;        // 20 questions
    return false;
  }
  function finish() {
    if (ST.timerInt) { clearInterval(ST.timerInt); ST.timerInt = null; }
  ...
  function tierMeta(t) {
    return t === 'exam_prep' ? {
      ...
      targetAr: 'للمتيمكنمين المستعدين لـ STEP و TOEFL و IELTS', targetEn: 'Advanced students targeting STEP, TOEFL, IELTS',
      featAr: ['مسارات التجهيز للاختبارات', 'اساله محاكاه للامتحان', 'وحدات مصطلحات مستهدفه', 'ترجمه فوريه ومراجعه كتابه'],
      ...
      targetAr: 'للمبتديين من الصفر خطوه بخطوه', targetEn: 'Absolute beginners starting from scratch',
      featAr: ['وصول كامل للوحه التحكم والدروس', 'دروس اساسيه غير متيمكنمه', 'سيناريوهات يوميه واقعيه', 'ترجمه فوريه وشرح بالعربي'],
  ...
  function cardHtml(r, tier, level) {
    ...
    if (r.weekly_live_classes > 0) {
      html += '<div style="margin:10px 0;padding:10px 12px;border:1px solid rgba(200,169,106,.35);border-radius:12px;background:rgba(200,169,106,.08);">'
        + '<b style="color:var(--gold);">' + r.weekly_live_classes + ' ' + esc(T('classesWeek')) + '</b><br>'
        + '<span style="font-size:.74rem;color:var(--text-muted);">' + esc(T('classesWeekNote')) + '</span></div>';
    } else if (tier === 'exam_prep') {
      html += '<div style="margin:10px 0;font-size:.76rem;color:var(--text-muted);">' + esc(ar ? 'ما فيها حصص مباشره بهالمده' : 'No live classes at this duration') + '</div>';
    }
    ...
    html += '</ul>';
    var waText = ar
      ? 'السلام عليكم، اختبرت مستواي بمنصه PEL وطلع لي ' + (window.PEL_ENGINE && PEL_ENGINE.levelInfo ? (PEL_ENGINE.levelInfo(level).ar || level) : level) + ' (' + (tier === 'exam_prep' ? 'متيمكنم' : 'مبتدي') + '). ابي اشترك بباقه ' + dl.ar + ' بـ ' + r.price + ' ريال. وش طريقه الدفع؟'
      : 'Hello, I took the PEL assessment and my level is ' + (window.PEL_ENGINE && PEL_ENGINE.levelInfo ? (PEL_ENGINE.levelInfo(level).en || level) : level) + ' (' + (tier === 'exam_prep' ? 'advanced' : 'beginner') + '). I want the ' + dl.en + ' plan (' + r.price + ' SAR). How do I pay?';
  ...
  function renderPaywall(result, earlyStop) {
    ...
    body.innerHTML =
      '<div style="text-align:center;">' +
        '<div class="ob-title" style="font-size:1.4rem;">' + esc(ar ? 'خطتك جاهزه' : 'Your plan is ready') + '</div>' +
        '<div style="margin:10px 0 4px;font-size:.9rem;color:var(--text-muted);">' + esc(T('yourLevel')) + ': <b style="color:var(--gold);">' + (window.PEL_ENGINE && PEL_ENGINE.levelInfo ? (ar ? (PEL_ENGINE.levelInfo(result.level).ar || result.level) : (PEL_ENGINE.levelInfo(result.level).en || result.level)) : result.level) + '</b></div>' +
      ...
      '<div style="margin:26px 0 0;padding:16px;border:1px dashed var(--border);border-radius:14px;text-align:center;">' +
        ...
        '<a href="' + waLink(ar ? 'السلام عليكم، ابي اشتري رصيد حصص مباشره اضافيه. وش المتوفر؟' : 'Hello, I want to buy extra live class credits. What is available?') + '" target="_blank" rel="noopener" class="btn-outline magnetic" data-magnetic style="margin-top:10px;display:inline-block;">' + esc(T('buyCreditsCta')) + '</a>' +
      '</div>' +
  ...
  var FALLBACK_PLANS = [
    { tier: 'start_from_zero', duration_months: 1, price: 800, weekly_live_classes: 1, featured: false },
    { tier: 'start_from_zero', duration_months: 2, price: 1000, weekly_live_classes: 1, featured: false },
    { tier: 'start_from_zero', duration_months: 3, price: 1500, weekly_live_classes: 1, featured: true },
    { tier: 'exam_prep', duration_months: 1, price: 1000, weekly_live_classes: 1, featured: false },
    { tier: 'exam_prep', duration_months: 2, price: 1300, weekly_live_classes: 2, featured: false },
    { tier: 'exam_prep', duration_months: 3, price: 2200, weekly_live_classes: 3, featured: true }
  ];
  var FALLBACK_QUESTIONS = [
    { code: 'q01', tier: 'beginner', level: 'A1', difficulty_rating: 1, question_en: 'She ___ to school every day.', question_ar: 'هي ___ للمعلمه كل يوم.', options: [{ en: 'go', ar: 'go' }, { en: 'goes', ar: 'goes' }, { en: 'going', ar: 'going' }, { en: 'gone', ar: 'gone' }], correct_index: 1 },
    ...
    { code: 'n01', tier: 'beginner', level: 'A2', difficulty_rating: 2, question_en: 'Choose the correct verb: While I ___ dinner, the phone rang.', question_ar: 'اختار الينعل الصح: While I ___ dinner, the phone rang.', options: [{ en: 'was cooking', ar: 'was cooking' }, { en: 'cooked', ar: 'cooked' }, { en: 'cook', ar: 'cook' }, { en: 'cooking', ar: 'cooking' }], correct_index: 0 },
    { code: 'n02', tier: 'beginner', level: 'B1', difficulty_rating: 3, question_en: 'Choose the correct verb: She ___ her phone. She cannot find it.', question_ar: 'اختار الينعل الصح: She ___ her phone. She cannot find it.', options: [{ en: 'has lost', ar: 'has lost' }, { en: 'lost', ar: 'lost' }, { en: 'is losing', ar: 'is losing' }, { en: 'loses', ar: 'loses' }], correct_index: 0 },
    { code: 'n03', tier: 'beginner', level: 'B1', difficulty_rating: 3, question_en: 'Choose the correct word: I ___ live in Riyadh, but I moved.', question_ar: 'اختار الكلمه الصح: I ___ live in Riyadh, but I moved.', options: [{ en: 'used to', ar: 'used to' }, { en: 'use to', ar: 'use to' }, { en: 'using to', ar: 'using to' }, { en: 'am used', ar: 'am used' }], correct_index: 0 },
    { code: 'n04', tier: 'exam_prep', level: 'B1', difficulty_rating: 3, question_en: 'Choose: If I ___ rich, I would travel the world.', question_ar: 'اختار: If I ___ rich, I would travel the world.', options: [{ en: 'were', ar: 'were' }, { en: 'was', ar: 'was' }, { en: 'am', ar: 'am' }, { en: 'be', ar: 'be' }], correct_index: 0 },
    { code: 'n05', tier: 'exam_prep', level: 'B2', difficulty_rating: 4, question_en: 'Choose: She said she ___ the report yesterday.', question_ar: 'اختار: She said she ___ the report yesterday.', options: [{ en: 'had finished', ar: 'had finished' }, { en: 'has finished', ar: 'has finished' }, { en: 'finished', ar: 'finished' }, { en: 'finishes', ar: 'finishes' }], correct_index: 0 },
    { code: 'n06', tier: 'exam_prep', level: 'B2', difficulty_rating: 4, question_en: 'Choose the best word: She made a good ___ on the interviewers.', question_ar: 'اختار افضل كلمه: She made a good ___ on the interviewers.', options: [{ en: 'impression', ar: 'انطباع' }, { en: 'expression', ar: 'تعبير' }, { en: 'impressive', ar: 'مبهر' }, { en: 'impress', ar: 'يبهر' }], correct_index: 0 }
  ];
  ...
  var TRUE_ZERO_QUESTIONS = [
    { code: 'tz01', tier: 'true_zero', level: 'A0', difficulty_rating: 1,
      ...
      question_ar: '«صباح الخير» - اختار النطق الصح بالحينجليزي',
    ...
    { code: 'tz02', tier: 'true_zero', level: 'A0', difficulty_rating: 1,
      ...
      question_ar: '«عشانفك» - اختار النطق الصح بالحينجليزي',
    ...
    { code: 'tz03', tier: 'true_zero', level: 'A0', difficulty_rating: 1,
      ...
      question_ar: '«شكرا لك» - اختار النطق الصح بالحينجليزي',
    ...
    { code: 'tz04', tier: 'true_zero', level: 'A0', difficulty_rating: 1,
      ...
      question_ar: '«وش اسمك» - اختار النطق الصح بالحينجليزي',
    ...
    { code: 'tz05', tier: 'true_zero', level: 'A0', difficulty_rating: 1,
      ...
      question_ar: '«فرصه مبسوطه» - اختار النطق الصح بالحينجليزي',
    ...
    { code: 'tz06', tier: 'true_zero', level: 'A0', difficulty_rating: 1,
      ...
      question_ar: '«وين الحمام» - اختار النطق الصح بالحينجليزي',
    ...
    { code: 'tz07', tier: 'true_zero', level: 'A0', difficulty_rating: 1,
      ...
      question_ar: '«بكذا» - اختار النطق الصح بالحينجليزي',
    ...
    { code: 'tz08', tier: 'true_zero', level: 'A0', difficulty_rating: 1,
      ...
      question_ar: '«ما فهمت» - اختار النطق الصح بالحينجليزي',
      options: [{ en: 'اي بدونت اندرستاند', ar: 'اي بدونت اندرستاند' }, { en: 'اي كان اندرستاند', ar: 'اي كان اندرستاند' }, { en: 'اي بدونت نو', ar: 'اي بدونت نو' }, { en: 'اي بدونت لايك', ar: 'اي بدونت لايك' }], correct_index: 0 },
    { code: 'tz09', tier: 'true_zero', level: 'A0', difficulty_rating: 1,
      question_en: 'Can you help me - pick the correct pronunciation',
      question_ar: '«تيمكنر تساعدني» - اختار النطق الصح بالحينجليزي',
    ...
    { code: 'tz10', tier: 'true_zero', level: 'A0', difficulty_rating: 1,
      ...
      question_ar: '«اشوفك لسهين» - اختار النطق الصح بالحينجليزي',
