...
(function () {
  ...
  function open() {
    ...
    ensureQuestions().then(function (qs) {
      ST.pool = (qs && qs.length) ? qs : FALLBACK_QUESTIONS;
      renderIntro();
    });
  }
  function close() {
  ...
  function finish() {
    ...
    if (ST.mode === 'true_zero') {
      ...
      result.track = track; result.level = level; result.mode = 'true_zero';
    }
    try { localStorage.setItem('pel_assessment', JSON.stringify(result)); } catch (e) {}
    saveAssessmentRpc(track, level);
    renderPaywall(result, earlyStop);
  }
  function saveAssessmentRpc(track, level) {
    try {
      fetch(SB_URL + '/rest/v1/rpc/student_save_assessment', {
        ...
        body: JSON.stringify({ p_track: track, p_level: level })
      }).catch(function () {});
    } catch (e) {}
  }
  ...
  function renderPaywall(result, earlyStop) {
    ...
    body.innerHTML =
      ...
      '<div class="pricing-grid" style="grid-template-columns:repeat(3,1fr);gap:16px;" dir="' + (ar ? 'rtl' : 'ltr') + '">' + recRows.map(function (r) { return cardHtml(r, rec, result.level); }).join('') + '</div>' +
      (otherRows.length ? '<div style="margin:26px 0 6px;font-size:.8rem;font-weight:700;color:var(--text-muted);">' + esc(T('chooseOther')) + '</div><div class="pricing-grid" style="grid-template-columns:repeat(3,1fr);gap:16px;" dir="' + (ar ? 'rtl' : 'ltr') + '">' + otherRows.map(function (r) { return cardHtml(r, other, result.level); }).join('') + '</div>' : '') +
  ...
  body.addEventListener('click', function (ev) {
    ...
    if (a === 'gate-no') { ST.mode = 'true_zero'; ST.pool = TRUE_ZERO_QUESTIONS; ST.target = 1; renderQuestion(); return; }
    if (a === 'opt') {
