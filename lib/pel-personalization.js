...
(function(){
  ...
  const ACADEMY_META = {
    ...
    'reading-corner':          { en:'Reading Corner', ar:'ركن القراوة', icon:'book', from:'#C8A96A', to:'#17171B', diff:'Intermediate', level:'B1' },
  ...
  /* ---------------- DB curriculum override ----------------
     Set once at login via setCurriculumOverride(student_curriculum()).
     While null every lookup uses the static mirror below; while set,
     the DB is the single source of truth for academies/lessons/order. */
  let DB_CURRICULUM = null;
  function lessonsFor(id){
    if(DB_CURRICULUM) return DB_CURRICULUM.map[id] || [];
    return ACADEMY_LESSONS[id] || [];
  }
  function metaForLesson(id){
    if(DB_CURRICULUM && DB_CURRICULUM.lessons[id]) return DB_CURRICULUM.lessons[id];
    return LESSON_META[id] || null;
  }
  ...
  const API = {
    ...
    lessonMeta: function(id){ return metaForLesson(id); },
    academyLessons: function(id){ return lessonsFor(id); },
    ...
    buildRoute: function(profile){
      ...
      const seen = {};
      // Track-led foundation stage: when the admin-curated tracks are in
      // the catalog, they define where this learner starts - not a hard
      // coded academy. Falls back to the static foundations pair.
      const tracks = API.dbTracks ? API.dbTracks() : [];
      const track = tracks.filter(t => t.academies && t.academies.length)
        .sort((a, b) => (LEVEL_IDX[a.level] != null ? LEVEL_IDX[a.level] : 1) - (LEVEL_IDX[b.level] != null ? LEVEL_IDX[b.level] : 1))
        .find(t => (LEVEL_IDX[t.level] != null ? LEVEL_IDX[t.level] : 1) >= startIdx);
      if(track){
        track.academies.forEach(aid => { seen[aid] = 1; });
        stages.push({
          key: 'track-' + track.id,
          title: { en: track.name_en || 'Your Path', ar: track.name_ar || 'مسارك' },
          level: track.level,
          theme: 'foundation',
          units: track.academies.slice(0, 3).map(aid => ({
            academyId: aid,
            lessonIds: lessonsFor(aid).slice(0, 6),
            level: (ACADEMY_META[aid] || {}).level || track.level
          })),
        });
      } else if(startIdx === 0){
        seen['english-foundations'] = 1;
        stages.push({ key:'start-here', title:{ en:'Start Here', ar:'ابدا من هنا' }, level:'A1', theme:'foundation', units:[ { academyId:'english-foundations', lessonIds:lessonsFor('english-foundations').slice(0,6), level:'A1' } ] });
      } else {
        seen['everyday-english'] = 1;
        stages.push({ key:'warm-up', title:{ en:'Warm Up', ar:'تسخين' }, level: profile.estimatedStartingLevel, theme:'foundation', units:[ { academyId:'everyday-english', lessonIds:lessonsFor('everyday-english').slice(0,6), level:'A2' } ] });
      }
      ...
      pick.slice(0, maxUnits).forEach((u, i) => {
        const meta = ACADEMY_META[u.academyId] || { en: u.academyId, ar: u.academyId };
        stages.push({
          key: 's' + (i + 2),
          title: { en: meta.en, ar: meta.ar },
          level: u.level,
          theme: i < themeUnits.length ? 'context' : 'focus',
          units: [ { academyId: u.academyId, lessonIds: lessonsFor(u.academyId).slice(0, 6), level: u.level } ],
        });
      });
  ...
  /* ---------------- DB curriculum override ----------------
     shape: { academies:[{ id,name_en,name_ar,icon,color_from,color_to,
                           difficulty,level,sort_order,
                           lessons:[{id,title_en,title_ar,minutes,level}] }] }
     After this, lessonMeta()/academyLessons()/ACADEMY_META all serve the
     DB catalog (Admin > Lessons is the editor). Statics remain the
     offline fallback. */
  API.setCurriculumOverride = function(data){
    try{
      const list = (data && data.academies) || [];
      const meta = {};
      const lessons = {};
      const map = {};
      list.forEach(function(a){
        ...
        meta[a.id] = {
          en: a.name_en || a.id,
          ar: a.name_ar || a.name_en || a.id,
          icon: a.icon || 'book-open',
          from: a.color_from || '#C8A96A',
          to: a.color_to || '#A88345',
          diff: a.difficulty || 'Beginner',
          level: a.level || 'A1'
        };
        const lids = [];
        ...
        meta[a.id].tags = a.tags || [];
        map[a.id] = lids;
      });
      DB_CURRICULUM = { meta: meta, lessons: lessons, map: map,
        tracks: data && Array.isArray(data.tracks) ? data.tracks : [] };
      /* swap exposed ACADEMY_META contents in place so any cached reference
         (admin courses view, app renderers) sees the new catalog */
      Object.keys(ACADEMY_META).forEach(function(k){ delete ACADEMY_META[k]; });
      Object.keys(meta).forEach(function(k){ ACADEMY_META[k] = meta[k]; });
      return true;
    }catch(e){ return false; }
  };
  API.hasCurriculumOverride = function(){ return !!DB_CURRICULUM; };

  /* Full DB lesson content (items + exercises + engine metadata), or null
     when the lesson is not in the DB catalog. Used by the app reader to
     render DB-driven lessons instead of the static library. */
  API.dbLesson = function(id){
    if(!DB_CURRICULUM) return null;
    const l = DB_CURRICULUM.lessons[id];
    if(!l || !l.items || !l.items.length) return null;
    return l;
  };
  /* Academy tags (goal/context/skill) from the DB catalog. */
  API.dbAcademyTags = function(id){
    if(!DB_CURRICULUM || !DB_CURRICULUM.meta[id]) return [];
    return DB_CURRICULUM.meta[id].tags || [];
  };
  /* Active tracks with their ordered academy ids. */
  API.dbTracks = function(){ return DB_CURRICULUM ? DB_CURRICULUM.tracks : []; };
