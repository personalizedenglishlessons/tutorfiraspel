...
(function(){
  ...
  function statusLabel(done){
    return done ? L('Complete','تم') : L('Start','ابدا');
  }
  ...
  function renderTools(){
    var tools = [
      {view:'vocabulary', en:'Vocabulary', ar:'المفردات', icon:'book-open'},
      {view:'pronunciation', en:'Pronunciation', ar:'النطق', icon:'mic'},
      {view:'grammar', en:'Grammar', ar:'القواعد', icon:'spell-check'},
      {view:'listening', en:'Listening', ar:'الاستماع', icon:'headphones'},
      {view:'speaking', en:'Speaking', ar:'المحادثه', icon:'messages-square'},
      {view:'reading', en:'Reading', ar:'القرايه', icon:'book-text'},
      {view:'writing', en:'Writing', ar:'الكتابه', icon:'pen-line'},
      {view:'review', en:'Smart Review', ar:'المراجعه الذعشانه', icon:'rotate-ccw'},
      {view:'bookmarks', en:'Bookmarks', ar:'المفضله', icon:'bookmark'}
    ];
    return '<div class="pel-path-tools">'+tools.map(function(t){
      return '<button class="pel-tool" data-goto-view="'+t.view+'">'+
  ...
  function render(){
    ...
    html += '<div class="pel-path-head">'+
      '<div class="pel-path-titles">'+
        ...
        '<p class="pel-path-sub">'+L(overall+' of '+totalLessons+' lessons complete', overall+' من '+totalLessons+' دروس مكتمله')+'</p>'+
      '</div>'+
      renderTools()+
    '</div>';
    ...
    // wire tool links
    v.querySelectorAll('.pel-tool[data-goto-view]').forEach(function(b){
      b.addEventListener('click', function(){ if(typeof goToView==='function') goToView(b.getAttribute('data-goto-view')); });
    });
