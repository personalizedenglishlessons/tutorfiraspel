/* =====================================================================
   ...
   FACTORY FORM: app.html calls window.PEL_DASH_LIFE_FACTORY(deps) inside
   its IIFE (deps: viewRenderers, escapeHtml, goToView, localizeChrome,
   toast, and getters for the mutable state accountPrefs(),
   completedLessons(), vocabStore(), PEL_BEGINNER_LESSONS()).
   ...
   Study Tools strip, and a live Word Burst card. All study tools are
   unlocked for all students; counts animate from real values already
   painted by the app). Respects prefers-reduced-motion
   ...
   ===================================================================== */
window.PEL_DASH_LIFE_FACTORY = function (Deps) {
  Deps = Deps || {};
  var escapeHtml = Deps.escapeHtml;
  var goToView = Deps.goToView;
  var localizeChrome = Deps.localizeChrome;
  var toast = Deps.toast;
  var viewRenderers = Deps.viewRenderers;
  function ar() {
    try {
      var ap = (typeof Deps.accountPrefs === 'function') ? Deps.accountPrefs() : null;
      if (ap && ap.lang === 'ar') return true;
  ...
  var STYLE = '' +
    ...
    /* ---- word burst card ---- */
    '.pel-dash-burst{position:relative;overflow:hidden;margin:0 0 22px;border:1px solid rgba(200,169,106,.16);}' +
  ...
  function countUp(el) {
    ...
    if (el.dataset.pelCounted === '1' && el.dataset.pelLastVal === String(target)) return;
    el.dataset.pelCounted = '1'; el.dataset.pelLastVal = String(target);
    if (reduced()) { return; }
  ...
  function observeRise() {
    ...
    cards = Array.prototype.filter.call(cards, function (c) { return !c.closest('.page-head'); });
  ...
  /* ---- study tools strip: all tools unlocked for all students ---- */
  function go(view) { try { if (typeof goToView === 'function') goToView(view); } catch (e) {} }

  var TOOLS = [
    { view: 'curriculum', en: 'Curriculum', ar: 'المنهج', icon: 'route', subEn: 'Your path', subAr: 'مسارك' },
    { view: 'vocabulary', en: 'Vocabulary', ar: 'المفردات', icon: 'book-open', subEn: 'Saved words', subAr: 'كلماتك' },
    { view: 'review', en: 'Smart Review', ar: 'المراجعه', icon: 'rotate-ccw', subEn: 'Due cards', subAr: 'بطاقات مستحقه' },
    { view: 'bookmarks', en: 'Bookmarks', ar: 'المفضله', icon: 'bookmark', subEn: 'Marked lessons', subAr: 'دروس مختاره' },
    { view: 'pronunciation', en: 'Pronunciation', ar: 'النطق', icon: 'mic', subEn: 'Speak & score', subAr: 'انطق وسجل' },
    { view: 'grammar', en: 'Grammar', ar: 'القواعد', icon: 'spell-check', subEn: 'Fix mistakes', subAr: 'صحح الاغلاط' },
    { view: 'listening', en: 'Listening', ar: 'الاستماع', icon: 'headphones', subEn: 'Hear & answer', subAr: 'اسمع وجاوب' },
    { view: 'reading', en: 'Reading', ar: 'القرايه', icon: 'book-text', subEn: 'Read passages', subAr: 'اقرا النصوص' },
    { view: 'writing', en: 'Writing', ar: 'الكتابه', icon: 'pen-line', subEn: 'Write & check', subAr: 'رتب وتفحص' },
    { view: 'speaking', en: 'Speaking', ar: 'المحادثه', icon: 'messages-square', subEn: 'Talk it out', subAr: 'تكلم' }
  ];
  var ICON_SVG = {
  ...
  function buildToolsStrip() {
    ...
    var hero = document.getElementById('heroCard') || home.querySelector('.pel-dash-burst') || document.getElementById('ccBlock');
    // Anchor after Word Burst (if it exists) or after ccBlock.
    // Order must be: ccBlock → Word Burst → Study Tools → subBanner.
    // If we anchor to ccBlock directly, tools-wrap lands between ccBlock and
    // Word Burst, which is wrong.
    var burst = home.querySelector('.pel-dash-burst');
    var anchorEl = burst || document.getElementById('ccBlock') || hero;
    var anchor = anchorEl ? anchorEl.nextSibling : null;
  ...
  function burstPool() {
    var p = [];
    try { var vs = (typeof Deps.vocabStore === 'function') ? Deps.vocabStore() : null; if (Array.isArray(vs) && vs.length) p = vs.slice(); } catch (e) {}
    var PBL = (typeof Deps.PEL_BEGINNER_LESSONS === 'function') ? Deps.PEL_BEGINNER_LESSONS() : null;
    if (!p.length && PBL) {
      Object.keys(PBL).forEach(function (slug) {
        var L = PBL[slug];
  ...
  function buildWordBurst() {
    var home = document.getElementById('view-home'); if (!home) return;
    var old = home.querySelector('.pel-dash-burst');
    if(old){
      // Card already exists — refresh vocabulary pool and restart timer.
      // Also reposition it after ccBlock in case tools-strip moved it.
      if(old._pelStop) old._pelStop();
      var pool = burstPool();
      if(!pool.length) return;
      var wEl = old.querySelector('.pel-dash-burst-word');
      var aEl = old.querySelector('.pel-dash-burst-ar');
      var trEl = old.querySelector('.pel-dash-burst-tr');
      // Ensure Word Burst sits right after ccBlock (before tools-wrap)
      var ccEl = document.getElementById('ccBlock');
      if(ccEl && ccEl.nextSibling !== old){
        ccEl.insertAdjacentElement('afterend', old);
      }
      if(wEl && aEl && trEl){
        var idx = Math.floor(Math.random() * pool.length);
        function repaint(){ var it = pool[idx % pool.length]; wEl.textContent = it.en; aEl.textContent = it.ar || ''; trEl.textContent = it.translit || ''; wEl.classList.remove('pel-dash-burst-word'); void wEl.offsetWidth; wEl.classList.add('pel-dash-burst-word'); }
        old._pelRepaint = repaint;
        repaint();
        if(old._pelStart) old._pelStart();
        return;
      }
      // Stale card — remove and rebuild
      old.remove();
    }
    ...
    card.innerHTML =
      '<div style="display:flex;align-items:center;gap:10px;margin-bottom:12px;">' +
        ...
        '<span class="eyebrow" style="margin:0;">' + L('Word Burst', 'كلمه الحين') + '</span>' +
        '<span class="pel-dash-next" style="margin-inline-start:auto;font-size:.72rem;color:var(--text-muted);cursor:pointer;">' + L('Next', 'الجا') + '</span>' +
      '</div>' +
      '<div style="display:flex;align-items:center;justify-content:space-between;gap:20px;flex-wrap:wrap;">' +
        '<div style="flex:1 1 200px;">' +
          '<div class="pel-dash-burst-word" style="font-size:2rem;font-weight:700;font-family:\'Fraunces\',serif;letter-spacing:-.01em;direction:ltr;line-height:1.2;"></div>' +
          '<div class="pel-dash-burst-tr" style="font-size:.9rem;color:var(--gold-soft);margin-top:4px;direction:ltr;"></div>' +
        '</div>' +
        '<div class="pel-dash-burst-ar arabic" style="font-size:1.6rem;margin-top:0;text-align:end;flex:0 1 auto;"></div>' +
      '</div>';
    // Insert Word Burst AFTER the hero card / ccBlock so the Start Lesson card stays at the top.
    // Use #ccBlock (the container) not #heroCard (a child inside it) so insertAdjacentElement
    // places Word Burst as a sibling of ccBlock, not inside it.
    var hero = document.getElementById('heroCard');
    var cc = document.getElementById('ccBlock');
    if (hero && hero.parentNode !== cc) hero.insertAdjacentElement('afterend', card);
    else if (cc) cc.insertAdjacentElement('afterend', card);
    else { var cc2 = document.getElementById('ccBlock'); if (cc2) cc2.insertAdjacentElement('afterend', card); else home.insertBefore(card, home.firstChild); }
  ...
  function decorate() {
    ...
    syncQuiet();
    try { addShimmers(); } catch(e) {}
    try { addFlame(); } catch(e) {}
    try { buildToolsStrip(); } catch(e) {}
    try { buildWordBurst(); } catch(e) {}
    try { enableTilt(); } catch(e) {}
    try { enableMagnetic(); } catch(e) {}
    try { observeRise(); } catch(e) {}
    try { relocalizeHomeSoon(); } catch(e) {}
    try { ['heroStreakNum', 'heroLessonsNum', 'heroHoursNum', 'heroReviewDaysNum'].forEach(countUp); } catch(e) {}
    // Enforce homepage element order after all dynamic inserts are done.
    // Order: page-head → ccBlock(hero) → Word Burst → Study Tools → subBanner → grid
    try {
      var pageHead = home.querySelector('.page-head');
      var ccBlock = document.getElementById('ccBlock');
      var burst = home.querySelector('.pel-dash-burst');
      var tools = home.querySelector('.pel-dash-tools-wrap');
      var subBanner = document.getElementById('subBanner');
      var grid = document.getElementById('pelIntelGrid');
      [pageHead, ccBlock, burst, tools, subBanner, grid].filter(Boolean).forEach(function(el){
        home.appendChild(el);
      });
    } catch(e) {}
