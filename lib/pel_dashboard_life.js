...
window.PEL_DASH_LIFE_FACTORY = function (Deps) {
  ...
  function buildWordBurst() {
    ...
    if(old){
      // Reposition to top if already exists
      var hero = home.querySelector('#heroCard');
      if(hero && old !== hero.previousSibling){ hero.parentNode.insertBefore(old, hero); }
      return;
    }
    ...
    // Insert Word Burst at the top of the homepage, right before the hero card
    var hero = home.querySelector('#heroCard');
