...
window.PEL_DASH_LIFE_FACTORY = function (Deps) {
  ...
  function buildWordBurst() {
    ...
    var burst = home.querySelector('#dailyBurstSection');
    if (burst) {
      // Insert Word Burst right after Daily Burst section (top of homepage)
      burst.parentNode.insertBefore(card, burst.nextSibling);
    } else {
      // Fallback: insert before hero card
