...
window.PEL_DASH_LIFE_FACTORY = function (Deps) {
  ...
  var STYLE = '' +
    ...
    /* ---- word burst floating orbs (was heroCard) ---- */
    '.pel-dash-burst .pel-dash-orb{position:absolute;border-radius:50%;background:radial-gradient(circle,rgba(200,169,106,.20),transparent 70%);pointer-events:none;filter:blur(.5px);}' +
    '.pel-dash-burst .pel-dash-orb.o1{width:120px;height:120px;top:-30px;right:8%;animation:pelDashDrift1 11s ease-in-out infinite;}' +
    '.pel-dash-burst .pel-dash-orb.o2{width:70px;height:70px;bottom:-18px;left:18%;animation:pelDashDrift2 9s ease-in-out infinite;}' +
    '.pel-dash-burst .pel-dash-orb.o3{width:46px;height:46px;top:24px;left:42%;animation:pelDashDrift3 7.5s ease-in-out infinite;}' +
    ...
    /* ---- animated aurora glow inside the word burst ---- */
    '.pel-dash-burst .pel-dash-aurora{position:absolute;inset:-30%;background:conic-gradient(from 0deg,rgba(200,169,106,0),rgba(200,169,106,.12),rgba(63,140,114,.06),rgba(216,189,131,.10),rgba(200,169,106,0));filter:blur(22px);animation:pelDashAurora 16s linear infinite;pointer-events:none;z-index:0;}' +
    '.pel-dash-burst>*{position:relative;z-index:1;}' +
    '@keyframes pelDashAurora{to{transform:rotate(360deg);}}' +
    'body.pel-life-off .pel-dash-burst .pel-dash-aurora{animation:none !important;opacity:.4;}' +
    ...
    '#view-home.pel-dash-lessonify .pel-dash-burst::after{' +
    ...
    'body.pel-life-off #view-home.pel-dash-lessonify .pel-dash-burst::after{animation:none !important;opacity:.4;}' +
  ...
  function addOrbs() {
    var hero = document.querySelector('.pel-dash-burst'); if (!hero || hero.querySelector('.pel-dash-orb')) return;
  ...
  function addFlame() {
    var s = document.getElementById('heroStreakNum') || document.querySelector('#view-profile #heroStreakNum'); if (!s || s.querySelector('.pel-dash-ember')) return;
  ...
  function doneCount() { try { var cl = (typeof Deps.completedLessons === 'function') ? Deps.completedLessons() : null; return (cl && cl.size) ? cl.size : 0; } catch (e) { return 0; } }
  ...
  function buildToolsStrip() {
    ...
    var hero = home.querySelector('.pel-dash-burst');
  ...
  function buildWordBurst() {
    ...
    if(old){
      // Reposition to right after page header if not already there
      var pageHead = home.querySelector('.page-head');
      var expected = pageHead ? pageHead.nextSibling : home.firstChild;
      if(old !== expected){ old.parentNode.insertBefore(old, expected); }
      return;
    }
    var pool = burstPool(); if (!pool.length) return;
    var card = document.createElement('div');
    card.className = 'card card-elevated hoverable pel-dash-tilt pel-dash-rise pel-dash-burst';
    card.setAttribute('data-pel-tilt', '1');
    card.style.cssText = 'padding:34px;position:relative;overflow:hidden;margin-bottom:22px;animation-delay:180ms;';
    card.innerHTML =
      '<div style="position:absolute; top:-120px; right:-100px; width:320px; height:320px; border-radius:50%; background:radial-gradient(circle, rgba(200,169,106,.16), transparent 70%); pointer-events:none;"></div>' +
      ...
      '</div>' +
      '<div class="pel-dash-burst-word" style="font-size:1.6rem;font-weight:700;font-family:\'Fraunces\',serif;letter-spacing:-.01em;position:relative;"></div>' +
      '<div class="pel-dash-burst-ar arabic" style="font-size:1.2rem;margin-top:4px;position:relative;"></div>' +
      '<div class="pel-dash-burst-tr" style="font-size:.82rem;color:var(--text-muted);margin-top:2px;direction:ltr;position:relative;"></div>';
    // Insert Word Burst after the page header (not before it)
    var pageHead = home.querySelector('.page-head');
    if (pageHead) pageHead.insertAdjacentElement('afterend', card);
  ...
  function addAurora() {
    var h = document.querySelector('.pel-dash-burst');
