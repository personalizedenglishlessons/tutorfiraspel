...
window.PEL_DASH_LIFE_FACTORY = function (Deps) {
  ...
  function buildToolsStrip() {
    ...
    var hero = document.getElementById('heroCard') || home.querySelector('.pel-dash-burst');
  ...
  function buildWordBurst() {
    ...
    if(old){
      // Reposition to right after hero card if not already there
      var hero = document.getElementById('heroCard');
      if(hero && old !== hero.nextElementSibling){ hero.parentNode.insertBefore(old, hero.nextSibling); }
      return;
    }
    ...
    // Insert Word Burst right AFTER the hero card
    var hero = document.getElementById('heroCard');
    if (hero) hero.insertAdjacentElement('afterend', card);
    else { var ph = home.querySelector('.page-head'); if (ph) ph.insertAdjacentElement('afterend', card); else home.insertBefore(card, home.firstChild); }
