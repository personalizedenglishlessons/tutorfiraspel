...
window.PEL_DASH_LIFE_FACTORY = function (Deps) {
  ...
  function buildWordBurst() {
    ...
    if(old){
      // Reposition to right before hero card if not already there
      var hero = document.getElementById('heroCard');
      if(hero && old !== hero.previousElementSibling){ hero.parentNode.insertBefore(old, hero); }
      return;
    }
    ...
    // Insert Word Burst right before the hero card (after page header + ccBlock)
    var hero = document.getElementById('heroCard');
    if (hero) hero.parentNode.insertBefore(card, hero);
