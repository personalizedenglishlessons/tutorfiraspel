  ...
  const RENDERERS = {
    ...
    db_translate(inner, act, ctx){
      ...
      if(hb) hb.onclick=()=>{ if(hr){
        var hintTxt = p.source_tr || '';
        if(!hintTxt){
          // Fallback: show first letter of each answer word as a hint
          var firstLetters = answerWords.map(function(w){ return w.charAt(0); }).join(' ');
          hintTxt = srcIsEn
            ? L('First letters: ','الحروف الاولى: ') + esc(firstLetters)
            : L('Build the same meaning. Start with the action word.','رتب نفس المعنى. ابدا بكلمة الفعل، بعدين كمل بقية الجملة.');
        } else {
          hintTxt = esc(hintTxt);
        }
        hr.innerHTML='<div class="pel-act-translit">'+hintTxt+'</div>';
