  ...
  const RENDERERS = {
    ...
    spell(inner, act, ctx){
      ...
      ctx.self.ready(ctx, L('Check','تحقق'), ()=>{
        ...
        var fb = correct ? '' : L('Correct spelling: ','التهجئة الصحيحة: ')+esc(it.en);
    ...
    db_spell(inner, act, ctx){
      ...
      ctx.self.ready(ctx, L('Check','تحقق'), ()=>{
        ...
        var fb = correct ? '' : L('Correct spelling: ','التهجئة الصحيحة: ')+esc(p.answer);
        ctx.self.mark(ctx, correct, false, fb);
      });
      ctx.btn.disabled = true;
    },
    ...
    writing_practice(inner, act, ctx){
      ...
      const check = ()=>{
        ...
        // Normalize common contractions (I'm = I am, don't = do not) so
        // minor form differences don't cause a fail. Research: Arabic
        // speakers struggle with English writing — overly strict matching
        // frustrates rather than teaches.
        const norm = s => (s||'').toLowerCase()
          .replace(/\bi'm\b/g,'i am').replace(/\bdon't\b/g,'do not').replace(/\bcan't\b/g,'cannot')
          .replace(/\bwon't\b/g,'will not').replace(/\bisn't\b/g,'is not').replace(/\baren't\b/g,'are not')
          .replace(/\bwasn't\b/g,'was not').replace(/\bweren't\b/g,'were not').replace(/\bhasn't\b/g,'has not')
          .replace(/\bhaven't\b/g,'have not').replace(/\bhadn't\b/g,'had not').replace(/\bdoesn't\b/g,'does not')
          .replace(/\bdidn't\b/g,'did not').replace(/\bcouldn't\b/g,'could not').replace(/\bwouldn't\b/g,'would not')
          .replace(/\bshouldn't\b/g,'should not').replace(/\bit's\b/g,'it is').replace(/\bthat's\b/g,'that is')
          .replace(/\bwhat's\b/g,'what is').replace(/\bhe's\b/g,'he is').replace(/\bshe's\b/g,'she is')
          .replace(/\bwe're\b/g,'we are').replace(/\bthey're\b/g,'they are').replace(/\byou're\b/g,'you are')
          .replace(/\bwe've\b/g,'we have').replace(/\bthey've\b/g,'they have').replace(/\byou've\b/g,'you have')
          .replace(/\bwe'll\b/g,'we will').replace(/\bthey'll\b/g,'they will').replace(/\byou'll\b/g,'you will')
          .replace(/\bwe'd\b/g,'we would').replace(/\bthey'd\b/g,'they would').replace(/\byou'd\b/g,'you would')
