  ...
  const PEL_BEGINNER = {
    ...
    'shopping-and-money': {
      ...
      dialogue: [
        ...
        { who: 'B', en: 'OK, eight.', ar: 'تمام، ثمانانيه.', translit: 'اوعشان، ايت.' },
      ],
    },
    'daily-routine': {
...
.pel-stage-body{flex:1;overflow-y:auto;display:flex;flex-direction:column;align-items:center;justify-content:flex-start;padding:30px 20px 110px;}
...
@media(max-width:900px){
  ...
  .pel-stage-body{padding:26px 16px 100px;}
...
@media(max-width:680px){
  ...
  .pel-stage-body{padding:22px 14px 100px;}
}`;
  ...
  const PEL_MIN_PAIRS = [
    { contrast:'/p/ vs /b/', target:'pen', confusion:'ben', ar:'قلم', tipAr:'صوت P يطلع بهوا من الشفايف، B صوتها من الحلق. جرب قول بن بعدين پن.' },
      ...
      turns:[
        {
          ...
          options:[
            ...
            { text:'Doctor. Me. Now.', ok:false, whyAr:'جمله غير مفهومه. الانجليزي يحتاج فاعل + فعل. قل: I need to see the doctor.', outcomeAr:'الموظفه: "Im sorry, I didnt understand."' },
          ],
        },
        {
        ...
        },
        {
          ...
          options:[
            ...
            { text:'Nothing.', ok:false, whyAr:'لازم ترد. الصمت مو مهذب في الانجليزي. قل: Thank you, goodbye.', outcomeAr:'الموظفه: "Hello? Are you there?"' },
          ],
        },
      ],
    },
  ...
  var PRON_HINTS = {
    ...
    'cheap': { beats:1, ar:'قول كذا: چيپ - CH بعدين P', avoid:'مو كذا لانه غلط: شيب' },
  ...
  var PRON_PATTERNS = [
    ...
    { test:/^str\w/i, hint:{ beats:1, ar:'STR مع بعض - ابدا بـ S بعدين T بعدين R بسرعه', avoid:'مو كذا لانه غلط: تضيف ا قبل الـ S' } },
    // s + stop clusters: Arabic speakers add ا/ا before s
    { test:/^(sp|st|sk|sm|sn)\w/i, hint:{ beats:1, ar:'ابدا بحرف S مع الحرف اللي بعده مباشره ببدون ما تضيف صوت قبلها', avoid:'مو كذا لانه غلط: تضيف ا قبل الـ S' } },
    // ph → F
      ...
      if(renderer){
        ...
        if(!isTeachingAct && TEACH_BEFORE[act.type] && !act._taught){
          ...
          try{
            ...
            inner.innerHTML = '<div class="pel-act-prompt">' + L('Learn first then practice','تعلم اول شي بعدين تمرن') + '</div>';
          }
    ...
    renderDone(){
      ...
      if(!masteryMet){
        body.innerHTML = `<div class="pel-stage-done pel-card-enter">
          ...
          <p>${L('Answer correctly on the production exercises - first try - to complete this lesson. Review the weak spots and try again.','جاوب صح على تمارين الانتاج من اول محاوله عشان تخلص الدرس. راجع نقاط ضعفك وحاول ثانيه.')}</p>
          <div class="pel-act-hint" style="margin-top:10px;">${L('Production (first try)','الانتاج (اول محاوله)')}: <b>${prodFirstOk}/${prodTotal}</b> &nbsp;·&nbsp; ${L('Needed','المطلوب')}: <b>${Math.ceil(MASTERY_MIN_OK*prodTotal)}</b></div>${selfCheckCount?`<div class="pel-act-hint" style="margin-top:4px;">${L('Self-check activities (no mic) are not counted.','تمارين التقييم الذلكتي (ببدون مايك) لا تُحتسب.')}</div>`:''}
      ...
      if(_doneCard){
        ...
        var _secs = 5;
        _cd.innerHTML = L('Auto-advancing in','الانتقال التلقايي خلال') + ' <b>' + _secs + '</b> ' + L('seconds','ثانيه');
        _doneCard.appendChild(_cd);
        _cdTimer = setInterval(function(){
          _secs--;
          if(_secs <= 0){ clearInterval(_cdTimer); }
          else { _cd.innerHTML = L('Auto-advancing in','الانتقال التلقايي خلال') + ' <b>' + _secs + '</b> ' + L('seconds','ثانيه'); }
        }, 1000);
  ...
  function connectionNoteFor(act){
    ...
    if(/\bwould\s+you\s+mind\b/i.test(text)){
      return 'Would you mind = هل تمانع (اطلب تهذيب شديد). بعدها كلمه فعل + ing. مثال: Would you mind opening = هل تمانع تفتح';
    }
    ...
    if(/\bwhat\b/i.test(text)){
      return 'What = شنو/ايش. تبدا بيها السوال. بعدها كلمه مساعده (do, are, is). الترتيب: What + كلمه مساعده + الشخص + كلمه فعل. مثال: What do you want = شنو تبي';
    }
    ...
    if(/\b(a|an|the)\s+(big|small|red|blue|green|white|black|new|old|good|nice|beautiful|happy|sad|tired|hungry|good|great|long|short|tall|young|old|rich|poor|strong|weak|fast|slow|clean|dirty|empty|full|open|close|hot|cold|warm|cool|dry|wet|hard|soft|heavy|light|sharp|dull|smooth|rough|flat|round|square|deep|shallow|wide|narrow|thick|thin|bright|dark)\s+\w+/i.test(text)){
      return 'بالعربي الصفه تجي لسه الاسم: سياره حمرا. بالانجليزي الصفه تجي قبل الاسم: a red car. الترتيب: a/an/the + كلمه وصف + الاسم. مثال: a big house = بيت كبير، the new car = السياره الجديده';
    }
    ...
    if(/\bno\b/i.test(text) && !/\bnot\b/i.test(text) && !/\bknow\b/i.test(text) && !/\bnow\b/i.test(text) && !/\bnoon\b/i.test(text) && !/\bsnow\b/i.test(text)){
      return 'no = لا (للرفض المستقل). not = ما/مو (للنفي داخل الجمله). العربي يستخدم لا للاثنين بس بالانجليزي مختلفين. مثال: No, thanks = لا، شكرا. I am not tired = انا مو تعبان';
    }
  ...
  function buildTeachHtml(act){
    ...
    var html = '<div class="pel-teach">' +
      '<div class="pel-act-prompt">' + L('Learn first then practice','تعلم اول شي بعدين تمرن') + '</div>';
  ...
  const RENDERERS = {
    ...
    translate(inner, act, ctx){
      ...
      let sel=null;
      inner.innerHTML = `<div class="pel-act-prompt">${L('What is this in English?','وش ذا بالانجليزي؟')}</div>
    ...
    db_translate(inner, act, ctx){
      ...
      if(hb) hb.onclick=()=>{ if(hr){
        ...
        if(p.source_tr){
          ...
          hintHTML = '<div class="pel-act-translit"><span class="arabic">رتب نفس المعنى. ابدا بكلمه الينعل، بعدين كمل بقيه الجمله.</span></div>';
        }
    ...
    minimal_pairs(inner, act, ctx){
      ...
      const tipAr = pair.tipAr || ('الينرق صوت وواحد. اسمع الكلمتين زين، بعدين قول الكلمه الصح.');
      inner.innerHTML = `
    ...
    intonation(inner, act, ctx){
      ...
      inner.innerHTML = `
        ...
        <div class="pel-act-hint arabic" style="margin-bottom:14px;">${L('Hear both sentences, then pick which is the question.','اسمع الجملتين، بعدين اختر اي وحده سوال.')}</div>
      ...
      ctx.self.ready(ctx, L('Check','تحقق'), ()=>{
        ...
        const why = correct
          ...
          : Lt('Questions use different word order: Are you...? Do you...? The question was ' + (questionFirst ? 'Sentence 1' : 'Sentence 2') + '.', 'الاساله بالانجليزي تغير ترتيب الكلمات: Are you...? Do you...? السوال كان ' + (questionFirst ? 'جمله 1' : 'جمله 2') + '.');
  ...
  const PROMPTS_DEF = {
    ...
    db_spell:['Spell the English word','اهجّ الكلمه بالانجليزي'], db_translate:['Translate the sentence','ترجم الجمله'], db_correct:['Which sentence is correct?','اي جمله صح؟'],
