...
window.PEL_STAGE_FACTORY = function (Deps) {
  ...
  const PEL_BEGINNER = {
    ...
    'numbers-1-10': {
      title: { en: 'One to Ten', ar: 'من واحد لعشره' },
      items: [
        {
        ...
        },
        {
          ...
          usage: { en: 'The number after one.', ar: 'الرقم اللي لسه واحد.' },
          sentences: [
    ...
    'telling-time': {
      ...
      items: [
        ...
        },
        {
          ...
          sentences: [
            { en: 'One o\'clock.', ar: 'الساعه واحده.', translit: 'ون اكلوك.' },
...
.pel-stage-body{flex:1;overflow-y:auto;scrollbar-width:thin;scrollbar-color:var(--border) transparent;display:flex;flex-direction:column;align-items:center;justify-content:flex-start;padding:30px 20px 110px;}
.pel-stage-body::-webkit-scrollbar{width:4px;}
.pel-stage-body::-webkit-scrollbar-track{background:transparent;}
.pel-stage-body::-webkit-scrollbar-thumb{background:var(--border);border-radius:2px;}
...
@media(max-width:680px){
  ...
  const PEL_MIN_PAIRS = [
    ...
    { contrast:'/p/ vs /b/', target:'park', confusion:'bark', ar:'متنزه', tipAr:'P فيها هوا، B ما فيها. حط يدك على شفايفك وحس بالفرق.' },
      ...
      turns:[
        {
          ...
          promptAr:'وش تقول؟',
          options:[
            { text:'I want see doctor.', ok:false, whyAr:'ناقص فعل. قل: I want to see the doctor. لازم to قبل الفعل.', outcomeAr:'الموظفه: "Sorry? Can you say that again?"' },
  ...
  function connectionNoteFor(act){
    ...
    if(/\bthere\s+(is|are)\b/i.test(text)){
      return 'there is = فيه (لواحد) · there are = فيهم (للكثر). نستخدمها لما نقول ان شي موجود. الترتيب: There is/are + الشي + المكان. مثال: There is a book on the table = فيه كتاب على الطاوله';
    }
    ...
    if(/\bwhat\b/i.test(text)){
      return 'What = وش/ايش. تبدا بيها السوال. بعدها كلمه مساعده (do, are, is). الترتيب: What + كلمه مساعده + الشخص + كلمه فعل. مثال: What do you want = وش تبي';
    }
    ...
    if(/\b(this|that|these|those)\b/i.test(text)){
      return 'this = ذس (قريب، واحد) · that = ذات (بعيد، واحد) · these = ذيز (قريب، كثر) · those = ذوز (بعيد، كثر). نشاور بها على الشي. الترتيب: this/that/these/those + الشي + كلمه فعل + باقي الجمله. مثال: This is a book = ذا كتاب';
    }
    // 14. Articles - a/an/the
    if(/\b\ba\s+[bcdfghjklmnpqrstvwxyz]\w*/i.test(text)){
      return 'a = واحد (قبل صوت ساكن). نحطها قبل الشي الواحد. الترتيب: a + الشي. مثال: a book = كتاب، a car = سياره';
    }
    if(/\ban\s+[aeiou]\w*/i.test(text)){
      return 'an = واحد (قبل صوت متحرك a/e/i/o/u). نحطها قبل الشي الواحد اللي يبدا بحرف متحرك. الترتيب: an + الشي. مثال: an apple = تفاحه، an egg = بيضه';
    }
    ...
    if(/\b(plays|goes|eats|works|lives|likes|wants|needs|runs|walks|talks|looks|sees|hears|feels|comes|does|says|sits|stands|reads|writes|drives|rides|flies|takes|makes|gives|gets|lets|sets|hits|fits|puts|cuts|costs|bets|bids|shuts|bursts|casts)\b/i.test(text)){
      return 'في الحاضر البسيط: اذا كان الشخص واحد (he/she/it) نزيد s على اخر كلمه الفعل. الترتيب: الشخص + كلمه فعل+s + باقي الجمله. مثال: He plays = هو يلعب، She works = هي تشتغل';
    }
    ...
    if(/^(go|turn|take|stop|wait|listen|look|open|close|come|sit|stand|bring|give|show|tell|put|let|keep|try|make|check|read|write|say)\b/i.test(text)){
      return 'صيغه الامر: كلمه الفعل ببدون شخص قبلها. نعطي تعليمات مباشره. مثال: Go straight = امشي مستقيم، Turn left = لف يسار، Open the door = افتح الباب';
    }
    ...
    if(/\b(always|usually|sometimes|never|often|rarely|seldom)\b/i.test(text)){
      return 'كلمات التكرار (always, usually, never) تجي قبل الفعل الاساسي: I always go. بس لسه is/are/am: She is always late. مثال: I never drink coffee = ما اشرب قهوه ابدا، He is always happy = هو دايماً مبسوط';
    }
    return '';
  }
  ...
  function grammarGlossary(act){
    ...
    if(connNote){
      if(connNote.indexOf('كلمه فعل') >= 0 || connNote.indexOf('كلمه الفعل') >= 0){
        add('verb', 'كلمه الفعل', 'كلمه تدل على فعل او حركه. مثال: go (يروح)، eat (ياكل)، play (يلعب)، work (يشتغل)');
      }
      ...
      if(connNote.indexOf('كلمه اشاره') >= 0 || connNote.indexOf('الاشاره') >= 0){
        add('dem', 'كلمات الاشاره', 'كلمات نشاور فيها على شي. this = ذس (قريب واحد)، that = ذات (بعيد واحد)، these = ذيز (قريب كثر)، those = ذوز (بعيد كثر)');
      }
    }
    ...
    if(lesson.grammar && lesson.grammar.rule){
      ...
      if(/question/.test(r)) add('quest', 'كلمات السوال', 'What = وش، Where = وين، When = متى، Who = من، Why = ليش، How = عشان');
  ...
  const RENDERERS = {
    ...
    spell(inner, act, ctx){
      ...
      ctx.self.ready(ctx, L('Check','تحقق'), ()=>{
        ...
        var fb = correct ? '' : L('Correct spelling: ','الكتابه الصحيحه: ')+esc(it.en);
    ...
    choose_natural_expression(inner, act, ctx){
      ...
      var qHtml = q.qEn
        ? `<div dir="ltr" style="text-align:start;">${esc(q.qEn)}</div>` + (q.qAr ? `<div class="pel-act-ar arabic" style="font-size:.9rem;font-weight:400;margin-top:4px;">${esc(q.qAr)}</div>` : '') + (q.qTr ? `<div class="pel-act-translit" style="font-size:.82rem;">${esc(q.qTr)}</div>` : '')
        : `<div dir="ltr" style="text-align:start;">${esc(q.q || '')}</div>`;
    ...
    db_spell(inner, act, ctx){
      ...
      ctx.self.ready(ctx, L('Check','تحقق'), ()=>{
        ...
        var fb = correct ? '' : L('Correct spelling: ','الكتابه الصحيحه: ')+esc(p.answer);
        ctx.self.mark(ctx, correct, false, fb);
      });
      ctx.btn.disabled = true;
    },
    db_translate(inner, act, ctx){
      ...
      if(hb) hb.onclick=()=>{ if(hr){
        ...
        if(p.source_tr){
          ...
          hintHTML = '<div class="pel-act-translit"><span class="arabic">رتب نفس المعنى. ابدا بكلمه الفعل، بعدين كمل بقيه الجمله.</span></div>';
        }
    ...
    minimal_pairs(inner, act, ctx){
      ...
      const tipAr = pair.tipAr || ('الفرق صوت واحد. اسمع الكلمتين زين، بعدين قول الكلمه الصح.');
      inner.innerHTML = `
  ...
  var TEACHING_CONTENT = {
    ...
    'an': {
      meaning: 'تعريف للشي الواحد (قبل حرف عله)',
      useWhen: 'تتكلم عن شي واحد يبدا بحرف عله. مثال: an apple = تفاحه',
    ...
    'were': {
      ...
      dontUseWhen: 'ما تستخدمها مع الواحد — استخدم was',
    ...
    'has': {
      ...
      useWhen: 'تتكلم عن امتلاك شي لواحد غايب. مثال: He has a car = عنده سياره',
