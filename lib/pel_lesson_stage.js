...
@media(max-width:680px){
  ...
  function buildSequence(lesson, lessonId){
    ...
    // 0b. Match vocab items with sentences from the lesson's conversation.
    //     DB lessons often store example sentences as separate sentence-type
    //     items (mapped to conversation by dbToLesson), not attached to vocab.
    //     Without this, fill_blank / arrange_words / speaking get one-word
    //     "sentences" that make no sense to the student.
    if(Array.isArray(conv) && conv.length){
      main.forEach(function(it){
        var hasReal = it.sentences && it.sentences.some(function(s){ return s.en && s.en.split(/\s+/).filter(Boolean).length>=2; });
        if(!hasReal){
          var matches = conv.filter(function(c){ return c.en && norm(c.en).indexOf(norm(it.en))!==-1; });
          if(matches.length){
    ...
    if(orderExs.length){
      ...
      var arrWords = arrangeTarget.en ? arrangeTarget.en.replace(/[^\w\s]/g,'').split(/\s+/).filter(Boolean) : [];
    ...
    if(fbSentence && fbSentence.en){
      var fbWords = fbSentence.en.replace(/[^\w\s]/g,'').split(/\s+/).filter(Boolean);
      if(fbWords.length >= 3 && fbWords.some(function(w){ return norm(w)===norm(fbItem.en); })){
    ...
    const dictSentence = dictCand.find(function(s2){ return s2 && s2.en && s2.en.split(/\s+/).filter(Boolean).length >= 2; })
    ...
    if(speakSentence && speakSentence.en && speakSentence.en.split(/\s+/).filter(Boolean).length>=2){
    ...
    if(gpSentence && gpSentence.en && gpSentence.en.split(/\s+/).filter(Boolean).length>=3){
    ...
    // 15. Review (active recall): today's items PLUS any spaced-repetition
    // items that are due from previous lessons (most overdue first).
    const reviewItems = main.slice();
    const srsDue = srsDueList(4).filter(function(x){ return !main.some(function(m2){ return norm(m2.en)===norm(x.en); }); });
    ...
    const MODE_BY_TYPE = {
      ...
      listen:'recognition', identify_heard:'recognition', concept:'recognition',
  ...
  const TYPE_LABELS_DEF = {
    ...
    concept:['Concept','قاعدة'], db_order:['Build','رتب'], db_spell:['Spell','تهجى'],
  ...
  const PROMPTS_DEF = {
    ...
    concept:['Read the rule','اقرأ القاعدة'], db_order:['Tap words in order','اضغط الكلمات بالترتيب'],
