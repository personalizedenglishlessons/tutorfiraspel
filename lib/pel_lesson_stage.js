...
@media(max-width:680px){
  ...
  var PRON_HINTS = {
    ...
    'live': { beats:1, ar:'مقطع واحد: لِڤ — آخرها V', avoid:'لا تقول: لِف' },
  ...
  function buildSequence(lesson, lessonId){
    ...
    // Add DB exercise target sentences to the pool (order/translate exercises)
    exs.forEach(function(e){
      if(e && e.payload){
        if(e.type === 'order' && e.payload.answer) exPool.push({ en: Array.isArray(e.payload.answer) ? e.payload.answer.join(' ') : e.payload.answer });
        if(e.type === 'translate' && e.payload.source) exPool.push({ en: e.payload.source });
      }
    });
    var conceptExamples = exPool.filter(function(s){ return goodSentence(s, 3); }).slice(0, 4);
