...
@media(max-width:680px){
  ...
  function buildSequence(lesson, lessonId){
    ...
    // 2. Recognize (multiple choice en -> ar) — test ALL items, not just main[0]
    if(main.length >= 2){
      main.forEach((it, i) => {
    ...
    // 4. Build sentences (arrange words) — distribute across items
    main.forEach((it, i) => {
      const target = it.sentences[0] || {en:it.en, ar:it.ar, translit:it.translit};
      seq.push({ type:'arrange_words', sentence:target, skill:'grammar' });
    });

    // 5. Fill the missing word — use main[1] instead of always main[0]
    ...
    // 6. Spell the word — use main[2] to practice a different item
    ...
    // 8. Listen + identify — use main[2] for variety
    ...
    // 9. Pronunciation — use main[0] (first word is most important)
    seq.push({ type:'pronunciation', item:main[0]||items[0], skill:'pronunciation' });

    // 10. Speaking (say the sentence) — use main[1]'s sentence for variety
  ...
  const RENDERERS = {
    ...
    pronunciation(inner, act, ctx){
      const it=act.item;
      // Use speech recognition when available; fall back to self-assessment.
      // Show the word and translit but NOT the Arabic meaning during the
      // pronunciation attempt — the student should focus on saying the word,
      // not reading the translation.
      let micActive = false; let attempts = 0; let passed = false;
      ...
      inner.innerHTML = `
        ...
        <div id="pelProMicWrap" style="margin-top:16px;">
          ${canRec ? `<button class="pel-act-play" id="pelProMic" style="width:100%;"><i data-lucide="mic" width="16" height="16"></i> ${L('Speak now','انطق الحين')}</button>
            <div id="pelProFb" class="pel-feedback" style="margin-top:12px;"></div>
      ...
      if(canRec){
        ...
        const onResult = (text) => {
          micActive=false; micBtn.classList.remove('speaking'); micBtn.disabled=false;
          micBtn.innerHTML=`<i data-lucide="mic" width="16" height="16"></i> ${L('Try again','حاول ثانية')}`;
          attempts++;
          const sim = (typeof levenshtein==='function') ? levenshtein(norm(text), norm(it.en)) : 99;
          const ratio = sim / Math.max(it.en.length, text.length);
          if(ratio <= 0.3 || norm(text)===norm(it.en)){
            passed=true;
            fb.className='pel-feedback show ok'; fb.textContent=L('Great pronunciation!','نطق ممتاز!');
            ctx.btn.disabled=false;
            if(typeof lucide!=='undefined') lucide.createIcons();
          } else {
            fb.className='pel-feedback show no';
            fb.textContent=L('Close! Try again.','قريب! حاول ثانية.');
            if(typeof lucide!=='undefined') lucide.createIcons();
          }
        };
        micBtn.onclick = () => {
          if(micActive) return;
          micActive=true; passed=false;
          micBtn.classList.add('speaking'); micBtn.disabled=true;
          micBtn.innerHTML=`<i data-lucide="mic-off" width="16" height="16"></i> ${L('Listening...','استمع...')}`;
          if(typeof lucide!=='undefined') lucide.createIcons();
          try{ startRecognition(onResult, onResult); }catch(e){ onResult(''); }
        };
        ctx.self.ready(ctx, L('Skip','تخطي'), ()=>ctx.self.mark(ctx, true));
      } else {
        $('#pelProSelf').onclick = ()=>{ ctx.btn.disabled=false; ctx.self.mark(ctx, true); };
    ...
    speaking(inner, act, ctx){
      ...
      inner.innerHTML = `
        ...
        <div id="pelSpkMicWrap" style="margin-top:16px;">
          ${canRec ? `<button class="pel-act-play" id="pelSpkMic" style="width:100%;"><i data-lucide="mic" width="16" height="16"></i> ${L('Speak now','انطق الحين')}</button>
            <div id="pelSpkFb" class="pel-feedback" style="margin-top:12px;"></div>
      ...
      if(canRec){
        ...
        const onResult = (text) => {
          micActive=false; micBtn.classList.remove('speaking'); micBtn.disabled=false;
          micBtn.innerHTML=`<i data-lucide="mic" width="16" height="16"></i> ${L('Try again','حاول ثانية')}`;
          const sim = (typeof levenshtein==='function') ? levenshtein(norm(text), norm(s.en)) : 99;
          const ratio = sim / Math.max(s.en.length, text.length);
          if(ratio <= 0.35 || norm(text)===norm(s.en)){
            fb.className='pel-feedback show ok'; fb.textContent=L('Great!','ممتاز!');
            ctx.btn.disabled=false;
          } else {
            fb.className='pel-feedback show no';
            fb.textContent=L('Close! Try again.','قريب! حاول ثانية.');
        ...
        $('#pelSpkSelf').onclick = ()=>{ ctx.btn.disabled=false; ctx.self.mark(ctx, true); };
        ctx.self.ready(ctx, L('I said it','نطقتها'), ()=>ctx.self.mark(ctx, true));
      }
    },
