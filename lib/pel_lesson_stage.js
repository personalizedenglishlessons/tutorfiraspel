...
@media(max-width:680px){
  ...
  function buildTeachHtml(act){
    ...
    if(sensitive){
      ...
      // Show lesson vocabulary that is NOT an exact answer
      var safeVocab = lessonVocab.filter(function(v){
        return v && v.en && !hiddenWords.some(function(h){ return norm(h) === norm(v.en); });
      }).slice(0, 5);
      if(safeVocab.length){
      ...
      // Show "Hear the words" only if there are safe (non-answer) words to hear
  ...
  const RENDERERS = {
    ...
    spell(inner, act, ctx){
      ...
      inner.innerHTML = `<div class="pel-act-prompt">${L('Spell the English word for','اهجّ الكلمة الانجليزية لـ')}:</div>
    ...
    identify_heard(inner, act, ctx){
      ...
      const _distr=shuffle(pool.filter(x=>x&&x.en&&norm(x.en)!==norm(it.en))).slice(0,3);
      const _seen=new Set(); const _dd=[]; for(const d of _distr){ const k=norm(d.en); if(!_seen.has(k)){ _seen.add(k); _dd.push(d); } }
      // Determine the audio text: prefer a real example sentence over the bare word.
      var audioText = (it.example && it.example.en && goodSentence(it.example, 3)) ? it.example.en : it.en;
      var useSentenceMode = audioText && goodSentence({en:audioText}, 3) && containsTerm(audioText, it.en);
      ...
      if(!allLong && opts.length >= 2 && qPlain){
        ...
        if(blankMatch){
          ...
          if(sentMatch){
            ...
            if(containsTerm(baseSentence, correctText)){
              contextualOpts = opts.map(function(o, i){
                var text = optText(o);
  ...
  // Escape text, then wrap the `key` substring in a highlight span.
  function highlightChoice(text, key){
    if(!text || !key) return esc(text||'');
    var lowerText = String(text).toLowerCase();
    var lowerKey = String(key).toLowerCase();
    var idx = lowerText.indexOf(lowerKey);
    if(idx < 0) return esc(text);
    var prefix = text.substring(0, idx);
    var match = text.substring(idx, idx + key.length);
    var suffix = text.substring(idx + key.length);
