...
@media(max-width:680px){
  ...
  var PRON_HINTS = {
    ...
    'those': { beats:1, ar:'مقطع واحد: دوز', avoid:'لا تقول: دو-ز' },
  ...
  function buildSequence(lesson, lessonId){
    ...
    //     Uses lesson.exampleSentences (DB sentence items) or conv as fallback.
    //     Teaches how the concept/pattern works in real sentences.
    var exPool = (lesson && Array.isArray(lesson.exampleSentences) && lesson.exampleSentences.length)
      ? lesson.exampleSentences : conv;
    var conceptExamples = exPool.filter(function(s){ return goodSentence(s, 3); }).slice(0, 3);
  ...
  const RENDERERS = {
    ...
    concept_examples(inner, act, ctx){
      ...
      const renderOne = () => {
        ...
        ctx.self.ready(ctx, L('Show meaning','اعرض المعنى'), ()=>{ if(!revealed){ reveal(); } else { idx++; renderOne(); } });
      };
      renderOne();
    },
