...
@media(max-width:680px){
  ...
  function repairActivity(act, lesson, items){
    ...
    if(act.type === 'fill_blank'){
      ...
      if(!act.sentence || !act.sentence.en){
        var fbIt = act.item;
        if(fbIt.sentences && fbIt.sentences[0]) act.sentence = fbIt.sentences[0];
        else if(fbIt.example && fbIt.example.en) act.sentence = fbIt.example;
        else return null;
      }
    }
    if(act.type === 'db_order'){
      ...
      if(!hasTokens && !hasAnswer) return null;
    }
    ...
    if(act.type === 'recognize' || act.type === 'challenge'){
      if(!act.item || !act.item.en) return null;
    }
  ...
  const RENDERERS = {
    ...
    db_order(inner, act, ctx){
      ...
      tk.querySelectorAll('.pel-token').forEach(b=>b.onclick=()=>{ b.remove(); placed.push(tokens[+b.dataset.i]); renderDrop(); });
      ctx.self.ready(ctx, L('Check','تحقق'), ()=>{
        var ansStr = Array.isArray(p.answer) ? p.answer.join(' ') : (p.answer||'');
        if(!ansStr) ansStr = tokens.join(' ');
