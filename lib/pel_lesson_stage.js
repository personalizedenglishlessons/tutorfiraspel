...
@media(max-width:680px){
  ...
  function buildTeachHtml(act){
    ...
    if(lesson.grammar && lesson.grammar.rule){
      html += '<div class="pel-concept-steps">' + splitToSteps(lesson.grammar.rule).map(function(s,i){
        return '<div class="pel-concept-step"><span class="pel-concept-num">' + (i+1) + '</span><div class="pel-concept-step-body"><div class="pel-concept-step-en">' + esc(s) + '</div></div></div>';
      }).join('') + '</div>';
    }
      ...
      lesson.dbNotes.forEach(function(n){
        ...
        if(noteText){
          html += '<div class="pel-concept-steps">' + splitToSteps(noteText).map(function(s,i){
            ...
            return '<div class="pel-concept-step"><span class="pel-concept-num">' + (i+1) + '</span><div class="pel-concept-step-body"><div class="' + cls + '">' + esc(s) + '</div></div></div>';
          }).join('') + '</div>';
        }
      });
    }
  ...
  const RENDERERS = {
    ...
    concept(inner, act, ctx){
      ...
      // items). Structured into numbered step cards instead of one dense
      // paragraph so the student's eyes have a clear order to follow.
      const n = act.note || {};
      const en = n.en || n.note_en || '';
      const arText = n.ar || '';
      const tr = n.tr || '';
      const enSteps = splitToSteps(en);
      const arSteps = splitToSteps(arText);
      const stepCount = Math.max(enSteps.length, arSteps.length, 1);
      let stepsHtml = '';
      for(let i=0;i<stepCount;i++){
        const eStep = enSteps[i] || '';
        const aStep = arSteps[i] || '';
        if(!eStep && !aStep) continue;
        stepsHtml += `
          <div class="pel-concept-step">
            <span class="pel-concept-num">${i+1}</span>
            <div class="pel-concept-step-body">
              ${eStep?`<div class="pel-concept-step-en">${esc(eStep)}</div>`:''}
              ${aStep?`<div class="pel-concept-step-ar arabic">${esc(aStep)}</div>`:''}
            </div>
          </div>`;
      }
      inner.innerHTML = `
        <div class="pel-act-prompt">${L('Read and understand','اقرا وفهم')}</div>
        <div class="pel-concept-steps">${stepsHtml}</div>
