...
window.PEL_STAGE_FACTORY = function (Deps) {
  ...
  const PEL_BEGINNER = {
      ...
      items: [
        {
          ...
          ipa: '/həˈloʊ/',
          example: { en: 'Hello, how are you?', ar: 'هلا، عشانك؟', translit: 'هلو، هاو ار يو؟' },
          ...
          sentences: [
            ...
            { en: 'Hello, how are you?', ar: 'هلا، عشانك؟', translit: 'هلو، هاو ار يو؟' },
        ...
        {
          en: 'how are you', ar: 'عشانك', translit: 'هاو ار يو', pos: 'phrase',
          ipa: '/haʊ ɑːr juː/',
          example: { en: 'Hi, how are you today?', ar: 'هلا، عشانك اليوم؟', translit: 'هاي، هاو ار يو توداي؟' },
          usage: { en: 'A friendly check-in. Reply with "I am fine, thank you."', ar: 'سوال ودّي. جاوب بـ "تمام، شكرا".' },
          sentences: [
            { en: 'How are you?', ar: 'عشانك؟', translit: 'هاو ار يو؟' },
            { en: 'How are you today?', ar: 'عشانك اليوم؟', translit: 'هاو ار يو توداي؟' },
            { en: 'Hello, how are you doing today?', ar: 'هلا، عشانك اليوم؟', translit: 'هلو، هاو ار يو دوينق توداي؟' },
          ],
        },
      ],
      dialogue: [
        { who: 'A', en: 'Hello! How are you today?', ar: 'هلا! عشانك اليوم؟', translit: 'هلو! هاو ار يو توداي؟' },
    ...
    'feelings-and-emotions': {
      ...
      dialogue: [
        { who: 'A', en: 'How are you?', ar: 'عشانك؟', translit: 'هاو ار يو؟' },
...
@media(max-width:680px){
  ...
  function buildConversation(lesson, lessonId){
    ...
    if(!conv) conv = [{who:'A',en:'Hello!',ar:'هلا!',translit:'هلو!'},{who:'B',en:'Hi, how are you?',ar:'هلا، عشانك؟',translit:'هاي، هاو ار يو؟'}];
  ...
  const PEL_WRITING_PROMPTS = [
    ...
    { ar:'رتب: عشانك؟', en:'How are you?', answer:'How are you?', hint:'Question mark in English is ?', keywords:['how are you','عشانك','greeting','تحيه'] },
  ...
  function connectionNoteFor(act){
    ...
    if(/\bhow\b/i.test(text) && !/\bhow\s+(much|many|old)\b/i.test(text)){
      return 'How = عشان. للسوال عن الطريقه او الحاله. الترتيب: How + كلمه مساعده + الشخص + ... مثال: How are you = عشانك';
    }
    // 12. Comparatives
  ...
  function grammarGlossary(act){
    ...
    if(lesson.grammar && lesson.grammar.rule){
      ...
      if(/question/.test(r)) add('quest', 'كلمات السوال', 'What = شنو، Where = وين، When = متى، Who = من، Why = ليش، How = عشان');
  ...
  function buildTeachHtml(act){
    ...
    if(sensitive){
      ...
      if(nextWord){
        ...
        if(tc){
          ...
          // How to avoid
          if(tc.howToAvoid) html += '<div style="padding:8px 10px;background:rgba(99,102,241,.08);border-radius:8px;border-right:3px solid #6366f1;"><div style="font-size:.72rem;color:#6366f1;margin-bottom:3px;">' + L('How to avoid','عشان تتجنبه') + '</div><span class="arabic" style="font-size:.85rem;line-height:1.6;">' + esc(tc.howToAvoid) + '</span></div>';
          // What to remember
    ...
    if(connNote && !taughtKeys['connection']){
      ...
      html += '<div class="pel-teach-section">' + L('How words connect','عشان تتصل الكلمات') + '</div>';
      ...
      const reveal = () => {
        ...
        if(revealEl){
          ...
          if(tc){
            ...
            if(tc.howToAvoid) tcHtml += '<div style="padding:8px 10px;background:rgba(99,102,241,.08);border-radius:8px;border-right:3px solid #6366f1;"><div style="font-size:.72rem;color:#6366f1;margin-bottom:3px;">' + L('How to avoid','عشان تتجنبه') + '</div><span class="arabic" style="font-size:.85rem;line-height:1.6;">' + esc(tc.howToAvoid) + '</span></div>';
    ...
    how_to_say(inner, act, ctx){
      ...
      inner.innerHTML = `
        <div style="text-align:center;padding:8px 0 4px;">
          <div style="font-size:.78rem;color:var(--gold-soft);margin-bottom:10px;text-transform:uppercase;letter-spacing:.06em;">${L('How to say it','عشان تنطقها')}</div>
      ...
      if(canRec){
        ...
        micBtn.onclick = () => {
          ...
          recordAndScore((heard, score, err)=>{
            ...
            let enc = '';
            if(sc != null){
              if(sc < 40) enc = `<div class="pel-act-hint arabic" style="color:var(--text-muted);">الغلط عادي، ذا عشان نتعلم. حاول مره ثانيه.</div>`;
      ...
      ctx.self.ready(ctx, L('Check','تحقق'), ()=>{
        ...
        if(fb) fb.innerHTML = correct
          ...
          : `<div style="color:var(--gold);font-weight:700;">× ${L('Try again','حاول ثانيه')}</div>`
            + `<div class="pel-act-hint" dir="ltr">${esc(en)}</div>`
            + `<div class="pel-act-hint arabic">الغلط عادي، ذا عشان نتعلم.</div>`;
        ctx.self.mark(ctx, correct);
      });
      ctx.btn.disabled = true;
    },
  ...
  var FUNCTION_WORDS = {
    ...
    'where':'وين','what':'وش','who':'منو','when':'متى','why':'ليش','how':'عشان',
