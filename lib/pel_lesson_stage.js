...
@media(max-width:680px){
  ...
  function buildSequence(lesson, lessonId){
    ...
    if(typeof PEL_INTONATION_EX !== 'undefined' && PEL_INTONATION_EX.length){
      var _lessonTextInt = (notes||[]).map(function(n){ return (n.en||'')+' '+(n.ar||''); }).join(' ');
      main.forEach(function(it){ _lessonTextInt += ' ' + (it.en||'') + ' ' + (it.example&&it.example.en||''); });
    ...
    if(typeof PEL_WRITING_PROMPTS !== 'undefined' && PEL_WRITING_PROMPTS.length){
      var _lessonTextWp = (notes||[]).map(function(n){ return (n.en||'')+' '+(n.ar||''); }).join(' ');
      main.forEach(function(it){ _lessonTextWp += ' ' + (it.en||'') + ' ' + (it.example&&it.example.en||''); });
    ...
    // Cap at 14 steps: keep essential activities, trim from the middle if too long.
    // Research shows 8-14 activities per lesson is optimal (Rosetta Stone, Babbel).
    // Always keep: first concept, learn activities, review, challenge.
    var MAX_STEPS = 14;
    if(_repaired.length > MAX_STEPS){
      // Identify must-keep activities by type
      var mustKeep = function(a){
        return a.type === 'concept' || a.type === 'learn' || a.type === 'how_to_say' ||
               a.type === 'recognize' || a.type === 'review' || a.type === 'challenge';
      };
      var kept = _repaired.filter(mustKeep);
      var optional = _repaired.filter(function(a){ return !mustKeep(a); });
      // If must-keeps alone exceed MAX, cap them too (keep first MAX-2 and last 2)
      if(kept.length >= MAX_STEPS){
        var last2 = kept.slice(-2);
        kept = kept.slice(0, MAX_STEPS - 2).concat(last2);
        _repaired = kept;
      } else {
        // Fill remaining slots with optional activities (prioritize earlier ones)
        var remaining = MAX_STEPS - kept.length;
        _repaired = kept.concat(optional.slice(0, remaining));
      }
    }
    return _repaired;
  }
