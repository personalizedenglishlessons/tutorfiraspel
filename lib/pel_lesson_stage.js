  ...
  const Stage = {
    ...
    renderDone(){
      ...
      body.innerHTML = `<div class="pel-stage-done pel-card-enter">
        ...
        ${(s.recTotal||s.prodTotal)?`<div class="pel-act-hint" style="margin-top:10px;">${L('Recognition','استرجاع')}: <b>${s.recOk||0}/${s.recTotal||0}</b> &nbsp;·&nbsp; ${L('Production','انتاج')}: <b>${s.prodOk||0}/${s.prodTotal||0}</b>${selfCheckCount?` &nbsp;·&nbsp; ${L('Self-check','تقييم ذاتي')}: <b>${selfCheckCount}</b>`:''}</div>`:''}
        <div class="pel-done-row">
