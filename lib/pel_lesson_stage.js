...
@media(max-width:680px){
  ...
  const PEL_WRITING_PROMPTS = [
    { ar:'اكتب: اسمي احمد.', en:'My name is Ahmed.', answer:'My name is Ahmed.', hint:'Start with capital letter.' },
  ...
  const RENDERERS = {
    ...
    intonation(inner, act, ctx){
      ...
      if(playQ) playQ.onclick = ()=>{ playQ.classList.add('pel-played'); play(question,'en-US'); };
      if(playS) playS.onclick = ()=>{ playS.classList.add('pel-played'); play(statement,'en-US'); };
      ...
      ctx.btn.disabled=true;
      ctx.self.ready(ctx, L('Check','تحقق'), ()=>{
        // Sentence 1 = question form, Sentence 2 = statement form
        const correct = sel === 0;
        opts.forEach((b,i)=>{
          b.classList.remove('selected');
        ...
        const why = correct
          ? L('Questions use different word order: Are you...? Do you...?','الاسئلة تستخدم ترتيب كلمات مختلف: Are you...? Do you...?')
          : L('Questions in English change word order. Listen for Are you...? or Do you...?','الاسئلة بالانجليزي تغير ترتيب الكلمات. اسمع Are you...? او Do you...?');
