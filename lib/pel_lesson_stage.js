...
@media(max-width:680px){
  ...
  function captureActivityDOM(){
    ...
    try{
      // Selected options (multiple-choice renderers)
      var sel = [];
      Stage.el.querySelectorAll('.pel-option.selected, .pel-tile.selected, .pel-step-opt.selected').forEach(function(el){
        var k = el.getAttribute('data-key') || el.getAttribute('data-en') || el.getAttribute('data-i') || el.textContent.trim().slice(0,120);
        if(k) sel.push(k);
      });
  ...
  function saveStageSession(){
    try{
      ...
      var acts = {};
      // Save current activity's DOM state
      var cur = captureActivityDOM();
      if(Object.keys(cur).length) acts[String(s.idx)] = cur;
    ...
    try{
      ...
      if(r.selected && r.selected.length){
        r.selected.forEach(function(key){
          Stage.el.querySelectorAll('.pel-option, .pel-tile, .pel-step-opt').forEach(function(el){
            var dk = el.getAttribute('data-key') || el.getAttribute('data-en') || el.getAttribute('data-i') || el.textContent.trim().slice(0,120);
