...
@media(max-width:680px){
  ...
  const RENDERERS = {
    ...
    fill_blank(inner, act, ctx){
      ...
      const distractors=dedupeOpts(shuffle([target, ...shuffle(itemsDistractor(act,it)).slice(0,2)]), 3);
    ...
    spell(inner, act, ctx){
      ...
      tk.querySelectorAll('.pel-token').forEach(b=>b.onclick=()=>{ b.disabled=true; b.style.opacity='0.3'; placed.push(scrambled[+b.dataset.i]); renderDrop(); ctx.btn.disabled=false; });
    ...
    conversation_response(inner, act, ctx){
      ...
      const distractors=dedupeOpts(shuffle(conv.filter((c,j)=>j!==i && j!==i+1).map(c=>c.en).filter(e=>e&&norm(e)!==norm(reply.en))), 2);
    ...
    complete_dialogue(inner, act, ctx){
      ...
      const opts=dedupeOpts(shuffle([target.en, ...shuffle(pool).slice(0,2).map(p=>p.en)]), 3); let sel=null;
    ...
    guided_production(inner, act, ctx){
      ...
      const bank = dedupeOpts(shuffle([key, ...shuffle(_gpDistractors).slice(0,2)]), 3);
      let sel=null;
      inner.innerHTML = `
    ...
    free_response(inner, act, ctx){
      ...
      const addBack=(tok)=>{ const btn=document.createElement('button'); btn.className='pel-token'; btn.textContent=tok; btn.onclick=()=>{ if(btn.disabled)return; btn.remove(); placed.push(tok); renderDrop(); }; tk.appendChild(btn); };
      tk.querySelectorAll('.pel-token').forEach(b=>b.onclick=()=>{ b.remove(); placed.push(bank[+b.dataset.i]); renderDrop(); ctx.btn.disabled=false; });
    ...
    challenge(inner, act, ctx){
      ...
      const opts=dedupeOpts(shuffle([it,...shuffle(pool.filter(x=>x&&x.en&&norm(x.en)!==norm(it.en))).slice(0,3)]), 4); let sel=null;
