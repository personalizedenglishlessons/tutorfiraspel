...
@media(max-width:680px){
  ...
  function connectionNoteFor(act){
    ...
    if(/\b(have|has|ve|s)\s+\w+(ed|en|wn|rd|t)\b/i.test(text) || /\bhave\s+been\b/i.test(text)){
      return 'هنا نستخدم have او has + كلمة فعل بصيغة الماضي (زي stopped او eaten). معناها شي صار في الماضي ولا زال مهم للحين. have مع I/we/you/they، has مع he/she/it. مثال: I have eaten = انا اكلت (ولا زال تأثيره موجود)';
    }
    ...
    if(/\bwhat\b/i.test(text)){
      return 'What = شنو/ايش. تبدا بيها السؤال. بعدها كلمة مساعدة (do, are, is). الترتيب: What + كلمة مساعدة + الشخص + كلمة فعل. مثال: What do you want = شنو تبي';
    }
    if(/\bwhere\b/i.test(text)){
      return 'Where = وين. للسؤال عن المكان. الترتيب: Where + كلمة مساعدة + الشخص + ... مثال: Where do you live = وين تسكن';
    }
    if(/\bwhen\b/i.test(text)){
      return 'When = متى. للسؤال عن الوقت. الترتيب: When + كلمة مساعدة + الشخص + ... مثال: When do you work = متى تشتغل';
    }
    if(/\bwho\b/i.test(text)){
      return 'Who = من. للسؤال عن الشخص. الترتيب: Who + كلمة مساعدة + الشخص + ... مثال: Who is he = من هذا';
    }
    if(/\bwhy\b/i.test(text)){
      return 'Why = ليش. للسؤال عن السبب. الجواب يبدا بـ because (لان). الترتيب: Why + كلمة مساعدة + الشخص + ... مثال: Why are you happy = ليش انت مبسوط';
    }
    if(/\bhow\b/i.test(text) && !/\bhow\s+(much|many|old)\b/i.test(text)){
      return 'How = كيف. للسؤال عن الطريقة او الحالة. الترتيب: How + كلمة مساعدة + الشخص + ... مثال: How are you = كيف حالك';
    }
    // 12. Comparatives
    if(/\b\w+er\s+than\b/i.test(text) || /\bmore\s+\w+\s+than\b/i.test(text)){
      return 'للمقارنة بين شيئين: الكلمة القصيرة + er + than (bigger than = اكبر من). او more + كلمة وصف + than (more beautiful than = اجمل من). مثال: This is bigger than that = هذا اكبر من ذاك';
    }
    ...
    if(/\b(plays|goes|eats|works|lives|likes|wants|needs|runs|walks|talks|looks|sees|hears|feels|comes|does|says|sits|stands|reads|writes|drives|rides|flies|takes|makes|gives|gets|lets|sets|hits|fits|puts|cuts|costs|bets|bids|shuts|bursts|casts)\b/i.test(text)){
      return 'في المضارع البسيط: اذا كان الشخص واحد (he/she/it) نزيد s على آخر كلمة الفعل. الترتيب: الشخص + كلمة فعل+s + باقي الجملة. مثال: He plays = هو يلعب، She works = هي تشتغل';
    }
    // 22. Questions with do/does
    if(/\b(do you|does he|does she|does it|do they|do we|does the)\b/i.test(text)){
      return 'للسؤال في المضارع البسيط نستخدم Do او Does قبل الشخص. Does مع he/she/it، Do مع I/you/we/they. الترتيب: Do/Does + الشخص + كلمة فعل. مثال: Do you work = انت تشتغل؟، Does he like = هو يحب؟';
    }
