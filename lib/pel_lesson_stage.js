  ...
  function captureActivityDOM(){
    ...
    try{
      ...
      Stage.el.querySelectorAll('.pel-option.selected, .pel-tile.selected, .pel-step-opt.selected').forEach(function(el){
        var k = el.getAttribute('data-key') || el.getAttribute('data-en') || el.textContent.trim().slice(0,120);
        if(k) sel.push(k);
      });
    ...
    try{
      ...
      if(r.selected && r.selected.length){
        r.selected.forEach(function(key){
          Stage.el.querySelectorAll('[data-key],[data-en]').forEach(function(el){
            if((el.getAttribute('data-key')||el.getAttribute('data-en')||'') === key){
