  ...
  const Stage = {
    ...
    mark(ctx, correct){
      ...
      try{
        ...
        if(act && !act.counted){
          act.counted = true;
          if(act.mode==='production'){ s.prodTotal=(s.prodTotal||0)+1; if(correct){ s.prodOk=(s.prodOk||0)+1; act.okTracked = true; } }
          else { s.recTotal=(s.recTotal||0)+1; if(correct){ s.recOk=(s.recOk||0)+1; act.okTracked = true; } }
        } else if(act && act.counted && correct && !act.okTracked){
          // Retry success upgrades the ok counter (totals stay once-per-activity),
          // so mastery gating rewards the eventual correct answer, not just
          // first attempts — matching how a teacher would grade a redo.
          act.okTracked = true;
    ...
    renderDone(){
      ...
      /* ---- Mastery-gated completion (audit item 5) ----
         ...
         the language, not just clicked through: with at least 3 production
         activities, >= 60% must be answered correctly (eventual-correct
         after a retry counts — see mark()). Below the bar the done screen
         becomes a "keep practicing" screen: no XP, no markLessonComplete,
         no server progression. The student can always leave via Back to
         path, so the gate can never trap anyone in a lesson. */
      const MASTERY_MIN_OK = 0.6, MASTERY_MIN_PROD = 3;
      const prodTotal = s.prodTotal||0, prodOk = s.prodOk||0;
      const masteryMet = prodTotal < MASTERY_MIN_PROD || (prodOk / prodTotal) >= MASTERY_MIN_OK;
      if(!masteryMet){
        body.innerHTML = `<div class="pel-stage-done pel-card-enter">
          <div class="crown">↻</div>
          <h2>${L('Almost there','قربت توصل')}</h2>
          <p>${L('Answer correctly on the production exercises to complete this lesson. Review the weak spots and try again.','جاوب صح على تمارين الإنتاج عشان تخلص الدرس. راجع نقاط ضعفك وحاول ثانية.')}</p>
          <div class="pel-act-hint" style="margin-top:10px;">${L('Production','انتاج')}: <b>${prodOk}/${prodTotal}</b> &nbsp;·&nbsp; ${L('Needed','المطلوب')}: <b>${Math.ceil(MASTERY_MIN_OK*prodTotal)}</b></div>
      ...
      try{ if(typeof markLessonComplete==='function') s.completionPromise = markLessonComplete(s.academyId, s.lessonId); }catch(e){}
      s.done=true;
  ...
  const RENDERERS = {
    ...
    free_response(inner, act, ctx){
      ...
      inner.innerHTML = `<div class="pel-act-prompt">${prompt}</div>
        ...
        <button class="btn-outline" id="pelFrReview" style="margin-top:10px;width:100%;"><span class="arabic">راجع كتابتي</span>، ${L('Review my writing','راجع كتابتي')}</button>
    ...
    concept(inner, act, ctx){
      ...
      inner.innerHTML = `
        ...
        ${ar?`<div class="pel-act-ar arabic" style="font-size:1.05rem;line-height:1.8;">${esc(ar)}</div>`:''}
