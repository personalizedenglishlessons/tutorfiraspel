...
@media(max-width:680px){
  ...
  function buildTeachHtml(act){
    ...
    if(sensitive){
      ...
      // Exclude both hidden answers AND words already taught in learn/learn_sentence
      var taughtWords = getTaughtWords();
      var allHidden = hiddenWords.concat(taughtWords);

      // Find the NEXT untaught word to teach individually with full info
      var nextWord = lessonVocab.find(function(v){
        return v && v.en && !allHidden.some(function(h){ return norm(h) === norm(v.en); });
      });

      if(nextWord){
        // Teach this word individually — meaning, translit, examples, usage
  ...
  // Collect words already taught in learn/learn_sentence steps before the
  // current activity. Used by buildTeachHtml to avoid repeating vocabulary
  ...
  function getTaughtWords(){
    ...
    for(var i = 0; i < s.idx; i++){
      ...
      // Also track words taught in the teach panel's "New word" section
