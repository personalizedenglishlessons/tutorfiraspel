...
@media(max-width:680px){
  ...
  function repairActivity(act, lesson, items){
    ...
    if(act.type === 'spell' || act.type === 'translate' || act.type === 'pronunciation' || act.type === 'free_response'){
      if(!act.item || !act.item.en) return null;
    }
    if(act.type === 'match'){
  ...
  function buildTeachHtml(act){
    ...
    if(sensitive){
      ...
      if(prereqs.length){
        html += '<div class="pel-teach-section">' + L('Before this question','قبل هذا السؤال') + '</div>';
      ...
      if(nextWord){
        ...
        if(tc){
          ...
          // Common mistake
          if(tc.commonMistake) html += '<div style="padding:8px 10px;background:rgba(245,158,11,.08);border-radius:8px;border-right:3px solid var(--gold);"><div style="font-size:.72rem;color:var(--gold);margin-bottom:3px;">' + L('Common mistake','خطأ شائع') + '</div><span class="arabic" style="font-size:.85rem;line-height:1.6;">' + esc(tc.commonMistake) + '</span></div>';
          // How to avoid
  ...
  const RENDERERS = {
    learn(inner, act, ctx){
      ...
      const reveal = () => {
        ...
        if(revealEl){
          ...
          if(tc){
            ...
            if(tc.commonMistake) tcHtml += '<div style="padding:8px 10px;background:rgba(245,158,11,.08);border-radius:8px;border-right:3px solid var(--gold);"><div style="font-size:.72rem;color:var(--gold);margin-bottom:3px;">' + L('Common mistake','خطأ شائع') + '</div><span class="arabic" style="font-size:.85rem;line-height:1.6;">' + esc(tc.commonMistake) + '</span></div>';
      ...
      inner.innerHTML = `
        <div class="pel-act-en">${highlightImportant(esc(it.en), terms)}</div>
    ...
    learn_sentence(inner, act, ctx){
      ...
      inner.innerHTML = `
        <div class="pel-act-en">${highlightImportant(esc(s.en), terms)}</div>
    ...
    spell(inner, act, ctx){
      ...
      const reEnable=()=>{ tk.querySelectorAll('.pel-token').forEach(b=>{ if(!placed.some((_,i)=>false)){ b.disabled=false; b.style.opacity='1'; } }); 
        // Re-enable all letters that aren't used yet
        ...
        tk.querySelectorAll('.pel-token').forEach(function(b){ b.disabled=false; b.style.opacity='1'; });
      };
      ...
      $('#pelSpellClear').onclick=()=>{ placed=[]; renderDrop(); reEnable(); };
      ctx.self.ready(ctx, L('Check','تحقق'), ()=>{
    ...
    translate(inner, act, ctx){
      ...
      // No typing required.
      const it=act.item;
      const distractors = shuffle(itemsDistractor(act, it)).slice(0,3);
      const opts = shuffle([it.en, ...distractors]).slice(0,4);
      let sel=null;
    ...
    listen(inner, act, ctx){
      ...
      $('#pelLsnPlay').onclick=()=>{ played=true; play(_heard.text,'en-US'); ctx.btn.disabled=false; };
      ctx.btn.disabled=true;
      ctx.self.ready(ctx, L('Continue','متابعة'), ()=>ctx.self.next());
    ...
    pronunciation(inner, act, ctx){
      const it=act.item;
      // Always show BOTH the mic button AND the "I said it" fallback.
      // Speech recognition often fails with accents, background noise, or
      // browser issues — the student must always have a manual way to proceed.
      // Threshold lowered to 30% so beginners with accents are not blocked.
      ...
      inner.innerHTML = `
        ...
        <div id="pelProMicWrap" style="margin-top:16px;">
          ...
          <button class="pel-act-play" id="pelProSelf" style="width:100%;margin-top:10px;"><i data-lucide="check" width="16" height="16"></i> ${L('I said it','نطقتها')}</button>
        </div>
      ...
      // "I said it" button — always available, always passes
      const selfBtn = $('#pelProSelf');
      if(selfBtn) selfBtn.onclick = ()=>{ revealAr(); ctx.btn.disabled=false; ctx.self.mark(ctx, true); };
      if(canRec){
        const micBtn=$('#pelProMic'); const fb=$('#pelProFb');
        micBtn.onclick = () => {
          if(typeof recordAndScore !== 'function'){ revealAr(); ctx.self.mark(ctx, true); return; }
          ...
          recordAndScore((heard, score, err)=>{
            ...
            revealAr();
            if(err){
              // Don't block on errors — show the message but let the student
              // continue with the "I said it" button (already visible).
              fb.innerHTML = `<div class="pel-act-hint" style="color:var(--gold);">${typeof micErrorMessage==='function'?esc(micErrorMessage(err)):L('Mic error','خطا بالمايك')}</div><div class="pel-act-hint">${L('No problem — tap "I said it" to continue.','ما في مشكلة — اضغط "نطقتها" للمتابعة.')}</div>`;
              return;
            }
            ...
            fb.innerHTML = `<div style="font-size:1.25rem; font-weight:700; color:${ok?'var(--success)':'var(--gold)'};">${sc!=null?sc:'-'}% ${ok?'✓':'×'}</div>`
              ...
              + (!ok?`<div class="pel-act-hint">${L('No problem — tap "I said it" to continue.','ما في مشكلة — اضغط "نطقتها" للمتابعة.')}</div>`:'');
    ...
    speaking(inner, act, ctx){
      ...
      inner.innerHTML = `
        ...
        <div id="pelSpkMicWrap" style="margin-top:16px;">
          ...
          : `<div class="pel-act-hint">${L('Say it out loud, then mark yourself.','نطقها بصوت عالي، بعدها قيّم نفسك.')}</div>`}
          <button class="pel-act-play" id="pelSpkSelf" style="width:100%;margin-top:10px;"><i data-lucide="check" width="16" height="16"></i> ${L('I said it','نطقتها')}</button>
        </div>
      ...
      const selfBtn2 = $('#pelSpkSelf');
      if(selfBtn2) selfBtn2.onclick = ()=>{ revealAr(); ctx.btn.disabled=false; ctx.self.mark(ctx, true); };
      if(canRec){
    ...
    grammar_correction(inner, act, ctx){
      ...
      inner.innerHTML = `<div class="pel-act-prompt">${L('Which sentence is correct?','اي جملة صح؟')}</div>
        ...
        <div class="pel-options" style="margin-top:14px;">${opts.map((o,i)=>`<button class="pel-option" data-i="${i}" data-choice-key="${esc(o.text)}"><span class="o-en">${esc(o.text)}</span></button>`).join('')}</div>`;
      const btns=inner.querySelectorAll('.pel-option');
      btns.forEach(b=>b.onclick=()=>{ btns.forEach(x=>x.classList.remove('selected')); b.classList.add('selected'); sel=+b.dataset.i; ctx.btn.disabled=false; });
      ctx.btn.disabled=true;
      ctx.self.ready(ctx, L('Check','تحقق'), ()=>{
    ...
    free_response(inner, act, ctx){
      ...
      $('#pelFrClear').onclick=()=>{ placed=[]; renderDrop(); 
      ...
      ctx.self.ready(ctx, L('Done','تم'), ()=>{
        ...
        const usesTarget = norm(built).indexOf(norm(it.en)) !== -1;
    ...
    concept(inner, act, ctx){
      ...
      // Examples section
      if(examples.length){
        html += '<div class="pel-concept-section-label">📝 ' + L('Examples','امثلة') + '</div>';
    ...
    db_spell(inner, act, ctx){
      ...
      inner.innerHTML = `<div class="pel-act-prompt">${L('Spell the word','اهجّ الكلمة')}</div>
        ...
        <div style="font-size:1.4rem;font-family:'Fraunces',serif;letter-spacing:.35em;margin-top:8px;color:var(--text-muted);">${esc(display)}</div>
        <input class="pel-input" id="pelDbsIn" style="margin-top:16px;" placeholder="${Lt('Type the word in English...','اكتب الكلمة بالانجليزي...')}" autocomplete="off" autocapitalize="off" />
    ...
    db_translate(inner, act, ctx){
      ...
      // never the answer.
      // Determine direction by inspecting the actual source text, not the
      // 'dir' field, which is inconsistently populated in the DB
      // (null, 'false', 'en2ar', 'en-ar', 'ar-en' all appear).
      const ex = act.ex; const p = (ex && ex.payload) || {};
      ...
      inner.innerHTML = `<div class="pel-act-prompt">${srcIsEn?L('Translate to Arabic','ترجم للعربي'):L('Translate to English','ترجم للانجليزي')}</div>
        ...
        <input class="pel-input" id="pelDbtIn" style="margin-top:16px;" placeholder="${srcIsEn?Lt('Type the Arabic translation...','اكتب الترجمة بالعربي...'):Lt('Type the English translation...','اكتب الترجمة بالانجليزي...')}" autocomplete="off" autocapitalize="off" />
  ...
  var TEACHING_CONTENT = {
    'I': {
      ...
      howToAvoid: 'تذكر: I + am دايماً مع بعض. ما تجمع بين I و is أو are',
    ...
    'the': {
      meaning: 'التعريف (الـ)',
      useWhen: 'تتكلم عن شي محدد معروف. مثال: the book = الكتاب',
      dontUseWhen: 'لا تستخدمها قبل شي عام. ما تقول "the books are good" اذا كنت تقصد كتب بشكل عام',
      commonMistake: 'استخدام the مع كل شي حتى ما يحتاج',
      howToAvoid: 'اسال نفسك: هل اتكلم عن شي محدد؟ اذا لا، ما يحتاج the',
      remember: 'the = للشي المحدد المعروف'
    },
    'this': {
    ...
    'these': {
      ...
      useWhen: 'تشاور على اشياء قريبة وجمع. مثال: These are my books = هذي كتبي',
    ...
    'those': {
      meaning: 'تلك / ذول (جمع بعيد)',
      useWhen: 'تشاور على اشياء بعيدة وجمع. مثال: Those are nice = تلك حلوة',
    ...
    'am': {
      meaning: 'من فعل الكون — تستخدم مع I فقط',
    ...
    'is': {
      meaning: 'من فعل الكون — تستخدم مع he/she/it',
      useWhen: 'تتكلم عن شخص واحد غايب او شي واحد. مثال: He is tall = هو طويل',
      dontUseWhen: 'لا تستخدمها مع I ولا مع جمع',
      commonMistake: 'استخدام is مع they بدل are',
      howToAvoid: 'he/she/it = is، I = am، we/you/they = are',
      remember: 'is = مع المفرد الغايب'
    },
    'are': {
      meaning: 'من فعل الكون — تستخدم مع we/you/they',
      useWhen: 'تتكلم عن جمع او عن المخاطب. مثال: You are my friend = انت صديقي',
      dontUseWhen: 'لا تستخدمها مع I ولا مع he/she/it',
      commonMistake: 'استخدام are مع he/she/it',
      howToAvoid: 'we/you/they = are، I = am، he/she/it = is',
      remember: 'are = مع الجمع والمخاطب'
    },
    'do': {
      meaning: 'فعل مساعد للأسئلة والنفي في الحاضر',
      useWhen: 'تسأل سؤال في الحاضر. مثال: Do you like tea? = تحب الشاي؟',
    ...
    'does': {
      meaning: 'فعل مساعد مع he/she/it في الحاضر',
      useWhen: 'تسأل عن شخص واحد غايب. مثال: Does he work? = هو يشتغل؟',
    ...
    'where': {
      meaning: 'وين / وين',
      useWhen: 'تسأل عن المكان. مثال: Where is the book? = وين الكتاب؟',
      dontUseWhen: 'لا تستخدمها للسؤال عن الزمان — استخدم when',
    ...
    'what': {
      meaning: 'وش / ايش',
      useWhen: 'تسأل عن الشي. مثال: What is this? = وش هذا؟',
      dontUseWhen: 'لا تستخدمها للسؤال عن المكان — استخدم where',
      commonMistake: 'خلط بين what و where',
      howToAvoid: 'what = للسؤال عن الشي نفسه، where = للمكان',
      remember: 'what = وش (للشي)'
    },
    'not': {
      meaning: 'مو / ما (نفي)',
      useWhen: 'تنفي شي. مثال: I am not tired = انا مو تعبان',
      dontUseWhen: 'لا تستخدمها لوحدها — دايماً مع فعل مساعد',
      commonMistake: 'وضع not في المكان الخطأ',
      howToAvoid: 'not تجي بعد الفعل المساعد: am not, is not, are not',
      remember: 'not = نفي، تجي بعد الفعل'
    },
    'in': {
      meaning: 'في (داخل)',
      useWhen: 'تتكلم عن شي داخل مكان. مثال: The book is in the bag = الكتاب في الشنطة',
      dontUseWhen: 'لا تستخدمها للشي فوق الشي — استخدم on',
      commonMistake: 'خلط بين in و on',
      howToAvoid: 'in = داخل شي، on = فوق شي',
      remember: 'in = داخل'
    },
    'on': {
      meaning: 'على / فوق',
      useWhen: 'تتكلم عن شي فوق شي. مثال: The cup is on the table = الكوب على الطاولة',
      dontUseWhen: 'لا تستخدمها للشي داخل شي — استخدم in',
      commonMistake: 'خلط بين on و in',
      howToAvoid: 'on = فوق، in = داخل',
      remember: 'on = فوق'
    },
    'can': {
      meaning: 'اقدر (للقدرة)',
    ...
    'my': {
      ...
      howToAvoid: 'my = خاصتي (ملكية)، I = انا (ضمير)',
      remember: 'my = ملكية، I = ضمير'
    },
    'your': {
      meaning: 'خاصتك / مالك',
      ...
      howToAvoid: 'your = ملكية، you = ضمير مخاطب',
      remember: 'your = ملكية المخاطب'
    },
    'a': {
      meaning: 'تعريف للشي الواحد (غير محدد)',
      useWhen: 'تتكلم عن شي واحد غير محدد. مثال: a book = كتاب',
      dontUseWhen: 'لا تستخدمها قبل كلمة تبدأ بحرف علة — استخدم an',
      commonMistake: 'استخدام a قبل كلمة تبدأ بحرف علة',
    ...
    'an': {
      ...
      useWhen: 'تتكلم عن شي واحد يبدأ بحرف علة. مثال: an apple = تفاحة',
  ...
  var FUNCTION_WORDS = {
    'do':'فعل مساعد للأسئلة','does':'فعل مساعد مع he/she/it','did':'فعل مساعد للماضي',
    'am':'من فعل الكون (I am)','is':'من فعل الكون (he/she/it)','are':'من فعل الكون (we/you/they)',
    'not':'نفي (مو)','no':'لا','yes':'ايه','please':'لو سمحت','thank':'شكر',
    'I':'انا','you':'انت/انتم','your':'خاصتك','my':'خاصتي','his':'خاصته','her':'خاصتها','our':'خاصتنا','their':'خاصتهم',
    'where':'وين','what':'وش','who':'منو','when':'متى','why':'ليش','how':'كيف',
    'the':'التعريف','a':'تعريف','an':'تعريف',
    'this':'هذا','that':'ذاك','these':'هذي','those':'تلك',
