...
@media(max-width:680px){
  ...
  const Stage = {
    ...
    renderDone(){
      ...
      body.innerHTML = `<div class="pel-stage-done pel-card-enter">
        ...
        <div class="pel-done-tools" style="margin-top:16px; display:flex; gap:10px; flex-wrap:wrap; justify-content:center;">
          <button class="pel-tool-link" id="pelDoneReview" style="background:none; border:1px solid var(--border); border-radius:10px; padding:8px 14px; font-size:.8rem; color:var(--text-secondary); cursor:pointer;">${L('Review words','راجع الكلمات')}</button>
          <button class="pel-tool-link" id="pelDoneVocab" style="background:none; border:1px solid var(--border); border-radius:10px; padding:8px 14px; font-size:.8rem; color:var(--text-secondary); cursor:pointer;">${L('Vocabulary','المفردات')}</button>
          <button class="pel-tool-link" id="pelDoneGrammar" style="background:none; border:1px solid var(--border); border-radius:10px; padding:8px 14px; font-size:.8rem; color:var(--text-secondary); cursor:pointer;">${L('Grammar','القواعد')}</button>
        </div></div>`;
      ...
      // Contextual tool links: let students jump to review tools from the
      // lesson completion screen, so tools are discovered in context.
      var _reviewBtn = $('#pelDoneReview');
      if(_reviewBtn) _reviewBtn.onclick = function(){ try{ if(typeof goToView==='function') goToView('review'); }catch(e){} };
      var _vocabBtn = $('#pelDoneVocab');
      if(_vocabBtn) _vocabBtn.onclick = function(){ try{ if(typeof goToView==='function') goToView('vocabulary'); }catch(e){} };
      var _grammarBtn = $('#pelDoneGrammar');
      if(_grammarBtn) _grammarBtn.onclick = function(){ try{ if(typeof goToView==='function') goToView('grammar'); }catch(e){} };
  ...
  function connectionNoteFor(act){
    ...
    if(/\b(have|has)\b/i.test(text) && !/\b(have|has)\s+\w+(ed|en|wn|rd)\b/i.test(text)){
      return 'have/has = عندي/عنده (للملكية). have مع I/we/you/they، has مع he/she/it. الترتيب: الشخص + have/has + الشي. مثال: I have a car = عندي سيارة. She has a phone = عندها جوال';
    }
    ...
    if(/\b(my|your|his|her|our|their)\b/i.test(text)){
      return 'كلمات الملكية: my = خاصتي · your = خاصتك · his = خاصته · her = خاصتها · our = خاصتنا · their = خاصتهم. تحطها قبل الشي. الترتيب: كلمة ملكية + الشي. مثال: my name = اسمي، your phone = جوالك';
    }
    ...
    if(/\bso\s+do\s+i\b/i.test(text)){
      return 'So do I = وانا كذلك (موافقة). نستخدمها لما نوافق على كلام احد. مثال: A: I like coffee. B: So do I = وانا كذلك';
    }
  ...
  function grammarGlossary(act){
    ...
    if(connNote){
      ...
      if(connNote.indexOf('كلمة ملكية') >= 0){
        add('poss', 'كلمات الملكية', 'كلمات تدل على ملك. my = خاصتي، your = خاصتك، his = خاصته، her = خاصتها، our = خاصتنا، their = خاصتهم');
      }
    ...
    if(lesson.grammar && lesson.grammar.rule){
      ...
      if(/auxiliary/.test(r)) add('aux2', 'فعل مساعد', 'كلمة تساعد نسوي بها سوال او نفي. مثال: do, does, am, is, are, have, has');
      if(/article/.test(r)) add('article', 'ادوات التعريف', 'a = واحد (قبل ساكن)، an = واحد (قبل متحرك)، the = الـ (للمحدد)');
      if(/preposition/.test(r)) add('prep', 'حروف الجر', 'كلمات تدل على مكان او وقت. in = في، on = على، at = عند');
      if(/possessive/.test(r)) add('poss2', 'ضماير الملكية', 'my = خاصتي، your = خاصتك، his = خاصته، her = خاصتها');
  ...
  function wordTypeFor(word, lesson){
    ...
    // Common pronouns
    if(/^(i|you|he|she|it|we|they|me|him|her|us|them)$/.test(w)) return 'ضمير';
    // Common verbs
    if(/^(am|is|are|was|were|have|has|had|do|does|did|will|would|can|could|should|shall|may|might|must)$/.test(w)) return 'فعل مساعد';
    ...
    // Articles
    if(/^(a|an|the)$/.test(w)) return 'اداة تعريف';
    // Prepositions
    if(/^(in|on|at|under|over|next to|near|behind|in front of|between|above|below|to|from|of|with|by|for|about|into|out of|through)$/.test(w)) return 'حرف جر';
    // Question words
    if(/^(what|where|when|who|why|how|which|whose)$/.test(w)) return 'كلمة سوال';
    // Demonstratives
    if(/^(this|that|these|those)$/.test(w)) return 'كلمة اشارة';
    // Possessive adjectives
    if(/^(my|your|his|her|its|our|their)$/.test(w)) return 'ضمير ملكية';
  ...
  function posLabel(pos){
    ...
    if(/^(adv|rb|rbr|rbs|adverb)$/.test(p)) return 'ظرف';
    if(/^(pron|prp|pdt|poss|prp\$)$/.test(p)) return 'ضمير';
    if(/^(det|dt|pdt)$/.test(p)) return 'اداة تعريف';
    if(/^(in|prep|preposition)$/.test(p)) return 'حرف جر';
    if(/^(cc|conj|conjunction)$/.test(p)) return 'حرف عطف';
  ...
  function buildTeachHtml(act){
    ...
    // Glossary: explain grammar terms used in this lesson
    ...
    html += '<div class="pel-teach-title">' + L('You will practice','بتتمرن على') + ': ' + esc(actLabel) + '</div>';

    // Full sentence/question with Arabic translation
    var sentence = extractActivitySentence(act);
    if(sentence){
      html += '<div class="pel-teach-section">' + L('Sentence / Question','الجملة / السوال') + '</div>';
  ...
  const RENDERERS = {
    ...
    listen(inner, act, ctx){
      ...
      $('#pelLsnPlay').onclick=()=>{ played=true; play((it.example&&it.example.en)||it.en,'en-US'); ctx.btn.disabled=false; };
      ctx.btn.disabled=true;
    ...
    identify_heard(inner, act, ctx){
      ...
      $('#pelIdPlay').onclick=()=>play((it.example&&it.example.en)||it.en,'en-US');
      const btns=inner.querySelectorAll('.pel-option');
      ...
      if(canRec){
        ...
        micBtn.onclick = () => {
          ...
          recordAndScore((heard, score, err)=>{
            ...
            revealAr();
            if(err){
              fb.innerHTML = `<div class="pel-act-hint" style="color:var(--danger);">${typeof micErrorMessage==='function'?esc(micErrorMessage(err)):L('Mic error','خطا بالمايك')}</div>`;
    ...
    concept(inner, act, ctx){
      ...
      const tr = n.tr || '';
      inner.innerHTML = `
        <div class="pel-act-prompt">${L('Read and understand','اقرا وفهم')}</div>
  ...
  const TYPE_LABELS_DEF = {
    ...
    listening_dictation:['Dictation','املاء'], guided_production:['Build it','ركبها'],
