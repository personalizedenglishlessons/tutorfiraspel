...
@media(max-width:680px){
  ...
  const RENDERERS = {
    ...
    db_spell(inner, act, ctx){
      ...
      tk.querySelectorAll('.pel-token').forEach(b=>b.onclick=()=>{ b.disabled=true; b.style.opacity='0.3'; placed.push(scrambled[+b.dataset.i]); renderDrop(); });
      $('#pelDbsClear').onclick=()=>{ placed=[]; renderDrop(); reEnable(); };
      ...
      ctx.self.ready(ctx, L('Check','تحقق'), ()=>{
        const accepted=[p.answer].concat(Array.isArray(p.accept)?p.accept:[]).map(function(s){ return norm(s).replace(/\s+/g,''); });
        const built = norm(placed.join('')).replace(/\s+/g,'');
