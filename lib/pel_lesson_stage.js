...
@media(max-width:680px){
  ...
  function buildSequence(lesson, lessonId){
    ...
    seq.push({ type:'guided_production', item:gpItem, sentence:(gpItem.sentences[1]||gpItem.sentences[0]), skill:'writing' });
  ...
  const Stage = {
    ...
    open(academyId, lessonId){
      ...
      // finishing, jump to the saved activity index.
      try{
        var saved = loadStagePos(academyId, lessonId);
        if(saved && typeof saved.i==='number' && saved.i>0 && saved.i<this.state.seq.length){
    ...
    mark(ctx, correct){
      ...
      try{
        ...
        if(act && !act.counted){
          ...
          // Retry success upgrades the DISPLAY counter only (prodOk/recOk).
          // The mastery gate below judges FIRST-ATTEMPT production
          // (prodFirstOk), so re-clicking an exercise until green cannot
          // satisfy the gate — the student must redo the lesson and produce
          // correctly up front.
          act.okTracked = true;
  ...
  /* ---- 5a. STAGE POSITION PERSISTENCE (per-activity resume) ----
     Saves {academyId, lessonId, idx, ts} to localStorage (pel_stage_pos)
     on every activity transition so the student resumes mid-lesson after
     a reload, browser close, or device switch. lsSet (injected dep) also
     fire-and-forget upserts to student_data in Supabase for cross-device.
     Cleared on lesson completion (mastery met) and on logout. */
  var STAGE_POS_KEY = 'pel_stage_pos';
  function saveStagePos(){
    try{
      var s = Stage.state; if(!s || !s.academyId || !s.lessonId) return;
      var pos = { a:s.academyId, l:s.lessonId, i:s.idx, ts:Date.now() };
      var raw = JSON.stringify(pos);
      if(typeof localStorage!=='undefined') localStorage.setItem(STAGE_POS_KEY, raw);
      // Sync to student_data (jsonb) for cross-device. Store as JSON string
      // so the restore code writes the string (not [object Object]).
  ...
  function loadStagePos(academyId, lessonId){
    try{
      ...
      // Handle double-stringified values from jsonb restore
  ...
  const RENDERERS = {
    ...
    recognize(inner, act, ctx){
      ...
      inner.innerHTML = `<div class="pel-act-prompt">${prompt}</div>
        <div class="pel-options" id="pelRecOpts">${opts.map((o,i)=>`<button class="pel-option" data-i="${i}"><span class="o-en arabic">${esc(o.ar)}</span></button>`).join('')}</div>`;
    ...
    identify_heard(inner, act, ctx){
      ...
      inner.innerHTML = `<div class="pel-act-prompt">${L('What did you hear?','وش سمعت؟')}</div>
        ...
        <div class="pel-options" id="pelIdOpts" style="margin-top:16px;">${opts.map((o,i)=>`<button class="pel-option" data-i="${i}"><span class="o-en">${esc(o.en)}</span></button>`).join('')}</div>`;
      ...
      ctx.btn.disabled=true;
      ctx.self.ready(ctx, L('Check','تحقق'), ()=>{ const correct=opts[sel]&&opts[sel].en===it.en; btns.forEach((b,i)=>{ if(opts[i].en===it.en){ b.classList.add('correct'); b.insertAdjacentHTML('beforeend', `<span class="o-ar arabic">${esc(opts[i].ar||'')}</span>`); } else if(i===sel) b.classList.add('wrong'); }); ctx.self.mark(ctx,correct); });
    },
    ...
    choose_natural_expression(inner, act, ctx){
      ...
      inner.innerHTML = `<div class="pel-act-prompt">${L('Choose the natural one','اختر الطبيعي')}</div>
        ...
        <div class="pel-options">${opts.map((o,i)=>`<button class="pel-option" data-i="${i}"><span class="o-en">${esc(optText(o))}</span></button>`).join('')}</div>`;
      const btns=inner.querySelectorAll('.pel-option');
      btns.forEach(b=>b.onclick=()=>{ btns.forEach(x=>x.classList.remove('selected')); b.classList.add('selected'); sel=+b.dataset.i; });
      ctx.btn.disabled=true;
      ctx.self.ready(ctx, L('Check','تحقق'), ()=>{ const correct=sel===correctIdx; btns.forEach((b,i)=>{ if(i===correctIdx){ b.classList.add('correct'); const sub=optSub(opts[i]); if(sub) b.insertAdjacentHTML('beforeend', `<span class="o-ar arabic">${esc(sub)}</span>`); } else if(i===sel) b.classList.add('wrong'); }); ctx.self.mark(ctx,correct); });
    },
    listening_dictation(inner, act, ctx){
      ...
      inner.innerHTML = `
        ...
        <input class="pel-input" id="pelDicIn" style="margin-top:16px;" placeholder="${L('Type the sentence you heard...','اكتب الجملة اللي سمعتها...')}" autocomplete="off" autocapitalize="off" />
    ...
    guided_production(inner, act, ctx){
      ...
      inner.innerHTML = `
        ...
        <div class="pel-options" style="margin-top:14px;">${bank.map((w,i)=>`<button class="pel-option" data-i="${i}">${esc(w)}</button>`).join('')}</div>`;
      const btns=inner.querySelectorAll('.pel-option');
    ...
    free_response(inner, act, ctx){
      ...
      inner.innerHTML = `<div class="pel-act-prompt">${prompt}</div>
        ...
        <textarea class="pel-input" id="pelFrIn" style="margin-top:16px;min-height:90px;" placeholder="${L('Write your own sentence...','اكتب جملتك...')}" autocomplete="off" autocapitalize="off"></textarea>
    ...
    challenge(inner, act, ctx){
      ...
      btns.forEach(b=>b.onclick=()=>{ btns.forEach(x=>x.classList.remove('selected')); b.classList.add('selected'); sel=+b.dataset.i; });
      ctx.btn.disabled=true;
      ctx.self.ready(ctx, L('Check','تحقق'), ()=>{ const correct=opts[sel]&&opts[sel].en===it.en; btns.forEach((b,i)=>{ if(opts[i].en===it.en){ b.classList.add('correct'); b.insertAdjacentHTML('beforeend', `<span class="o-ar">${esc(opts[i].en)} · ${esc(opts[i].translit||'')}</span>`); } else if(i===sel) b.classList.add('wrong'); }); ctx.self.mark(ctx,correct); });
    },
    ...
    db_order(inner, act, ctx){
      ...
      inner.innerHTML = `<div class="pel-act-prompt">${L('Build the sentence','رتب الجملة')}</div>
        ...
        <div class="pel-arrange-drop" id="pelDboDrop" data-ph="${L('Tap words below','اضغط الكلمات تحت')}"></div>
    ...
    db_spell(inner, act, ctx){
      ...
      inner.innerHTML = `<div class="pel-act-prompt">${L('Spell it','املكها')}</div>
        ...
        <input class="pel-input" id="pelDbsIn" style="margin-top:16px;" placeholder="${L('Type the word...','اكتب الكلمة...')}" autocomplete="off" autocapitalize="off" />
    ...
    db_translate(inner, act, ctx){
      ...
      inner.innerHTML = `<div class="pel-act-prompt">${srcIsEn?L('Say it in Arabic','قلها بالعربي'):L('Say it in English','قلها بالانجليزي')}</div>
        ...
        <input class="pel-input" id="pelDbtIn" style="margin-top:16px;" placeholder="${srcIsEn?L('Type in Arabic...','اكتب بالعربي...'):L('Type in English...','اكتب بالانجليزي...')}" autocomplete="off" autocapitalize="off" />
    ...
    db_correct(inner, act, ctx){
      ...
      inner.innerHTML = `<div class="pel-act-prompt">${L('Pick the correct one','اختر الصح')}</div>
        ...
        <div class="pel-options">
          <button class="pel-option" data-ok="0">${esc(wrong)}</button>
          <button class="pel-option" data-ok="1">${esc(right)}</button>
        </div>`;
      const btns=inner.querySelectorAll('.pel-option');
