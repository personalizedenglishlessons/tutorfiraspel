...
@media(max-width:680px){
  ...
  const PEL_INTONATION_EX = [
    { sentence:'You are coming', rising:'You are coming?', falling:'You are coming.',
      ar:'انت جاي', risingMeaning:'سؤال', fallingMeaning:'جملة خبرية' },
    { sentence:'She is here', rising:'She is here?', falling:'She is here.',
      ar:'هي هنا', risingMeaning:'سؤال', fallingMeaning:'جملة خبرية' },
    { sentence:'You like it', rising:'You like it?', falling:'You like it.',
      ar:'يعجبك', risingMeaning:'سؤال', fallingMeaning:'جملة خبرية' },
    { sentence:'They are ready', rising:'They are ready?', falling:'They are ready.',
      ar:'هم جاهزين', risingMeaning:'سؤال', fallingMeaning:'جملة خبرية' },
  ];
  ...
  const PEL_WRITING_PROMPTS = [
    { ar:'اكتب: انا اسمه احمد.', en:'My name is Ahmed.', answer:'My name is Ahmed.', hint:'Start with capital letter.' },
    { ar:'اكتب: انا من السعودية.', en:'I am from Saudi Arabia.', answer:'I am from Saudi Arabia.', hint:'Capitalize country name.' },
    { ar:'اكتب: كيف حالك؟', en:'How are you?', answer:'How are you?', hint:'Question mark in English is ?' },
    { ar:'اكتب: شكرا جزيلا.', en:'Thank you very much.', answer:'Thank you very much.', hint:'Capitalize the first letter.' },
    { ar:'اكتب: انا طالب.', en:'I am a student.', answer:'I am a student.', hint:'Use a before student.' },
    { ar:'اكتب: انا ابي قهوة.', en:'I want coffee.', answer:'I want coffee.', hint:'No need for please here.' },
    { ar:'اكتب: وين الحمام؟', en:'Where is the bathroom?', answer:'Where is the bathroom?', hint:'Start with Where.' },
    { ar:'اكتب: كم الساعة؟', en:'What time is it?', answer:'What time is it?', hint:'Question form.' },
  ];
  ...
  function buildSequence(lesson, lessonId){
    ...
    if(typeof PEL_MIN_PAIRS !== 'undefined' && PEL_MIN_PAIRS.length){
      var _mpWord = ((main[0]||items[0])||{}).en || '';
      var _mpMatch = PEL_MIN_PAIRS.find(function(p){ return p.target.toLowerCase() === _mpWord.toLowerCase(); });
      if(_mpMatch){
        seq.push({ type:'minimal_pairs', pair:_mpMatch, skill:'pronunciation' });
      } else {
        // Rotate through pairs on lessons without a direct match
        var _mpRot = PEL_MIN_PAIRS[_mpRotIdx % PEL_MIN_PAIRS.length];
        _mpRotIdx++;
        seq.push({ type:'minimal_pairs', pair:_mpRot, skill:'pronunciation' });
      }
    }
    ...
    // 10b. Intonation Exercise — identify meaning from intonation pattern.
    //      Research: Arabic speakers struggle with English intonation.
    //      Rotates through the set, one per lesson.
    if(typeof PEL_INTONATION_EX !== 'undefined' && PEL_INTONATION_EX.length){
      ...
      _intRotIdx++;
      seq.push({ type:'intonation', sentence:_intEx.sentence, rising:_intEx.rising,
        falling:_intEx.falling, ar:_intEx.ar, risingMeaning:_intEx.risingMeaning,
        fallingMeaning:_intEx.fallingMeaning, skill:'listening' });
    }
    ...
    if(typeof PEL_SCENARIO_TASKS !== 'undefined' && PEL_SCENARIO_TASKS.length){
      ...
      var _scenario = _scenarioMatch || PEL_SCENARIO_TASKS[_scenarioRotIdx % PEL_SCENARIO_TASKS.length];
      _scenarioRotIdx++;
    ...
    //      One per lesson, rotates through the set.
    if(typeof PEL_WRITING_PROMPTS !== 'undefined' && PEL_WRITING_PROMPTS.length){
      var _wpPrompt = PEL_WRITING_PROMPTS[_wpRotIdx % PEL_WRITING_PROMPTS.length];
      _wpRotIdx++;
      seq.push({ type:'writing_practice', prompt:_wpPrompt, skill:'writing', mode:'production' });
    }
  ...
  const RENDERERS = {
    ...
    minimal_pairs(inner, act, ctx){
      ...
      const canRec = typeof recordAndScore === 'function';
      let attempted = false;
      ...
      inner.innerHTML = `
        ...
        <div style="display:flex;gap:12px;margin-bottom:16px;">
          <div style="flex:1;text-align:center;padding:16px;border:2px solid var(--success);border-radius:12px;background:rgba(46,204,113,.06);">
            ...
            <div class="arabic" style="font-size:.85rem;color:var(--text-muted);">${esc(ar)}</div>
            <button class="pel-act-play" id="mpPlayT"><i data-lucide="volume-2" width="16" height="16"></i> ${L('Hear','اسمع')}</button>
          </div>
          <div style="flex:1;text-align:center;padding:16px;border:2px solid var(--error);border-radius:12px;background:rgba(231,76,60,.06);">
        ...
        <div style="text-align:center;margin-bottom:12px;">
          <button class="pel-act-primary" id="mpMic" style="background:var(--error);color:#fff;border:none;padding:12px 24px;border-radius:10px;font-size:1rem;cursor:pointer;">
            <i data-lucide="mic" width="18" height="18"></i> ${L('Say the correct word','نطق الكلمة الصح')}
          </button>
        </div>
        <div id="mpFb"></div>
        `:''}`;
    ...
    // ---- Intonation Exercise: identify meaning from intonation ----
    // Research: Arabic speakers struggle with English intonation and stress
    // patterns, which can cause misunderstandings.
    // Ref: https://www.e-iji.net/dosyalar/iji_2026_3_19.pdf
    intonation(inner, act, ctx){
      const sentence = act.sentence || '';
      const ar = act.ar || '';
      const rising = act.rising || sentence + '?';
      const falling = act.falling || sentence + '.';
      const risingMeaning = act.risingMeaning || L('A question','سؤال');
      const fallingMeaning = act.fallingMeaning || L('A statement','جملة خبرية');
      let played = false;
      let sel = null;

      inner.innerHTML = `
        <div class="pel-act-prompt">${L('Listen to the intonation','اسمع النبرة')}</div>
        <div class="pel-act-hint arabic" style="margin-bottom:14px;">${L('Hear both versions, then pick what each one means.','اسمع النسختين، بعدين اختر وش معنى كل واحدة.')}</div>
        <div style="text-align:center;margin-bottom:16px;">
          <button class="pel-act-play" id="intPlayR" style="margin:0 6px;"><i data-lucide="volume-2" width="16" height="16"></i> ${L('Version 1','نسخة 1')}</button>
          <button class="pel-act-play" id="intPlayF" style="margin:0 6px;"><i data-lucide="volume-2" width="16" height="16"></i> ${L('Version 2','نسخة 2')}</button>
        </div>
        <div class="pel-act-hint" style="margin-bottom:14px;" dir="ltr">"${esc(sentence)}"</div>
        ${ar?`<div class="pel-act-hint arabic" style="margin-bottom:14px;">${esc(ar)}</div>`:''}
        <div class="pel-options" id="intOpts">
          <button class="pel-option" data-i="0" data-choice-key="rising"><span class="arabic">${esc(risingMeaning)}</span></button>
          <button class="pel-option" data-i="1" data-choice-key="falling"><span class="arabic">${esc(fallingMeaning)}</span></button>
        </div>`;

      const playR = inner.querySelector('#intPlayR');
      const playF = inner.querySelector('#intPlayF');
      const opts = inner.querySelectorAll('.pel-option');

      // Play rising intonation (question) then falling (statement)
      if(playR) playR.onclick = ()=>{ played=true; playR.classList.add('pel-played'); play(rising,'en-US'); };
      if(playF) playF.onclick = ()=>{ played=true; playF.classList.add('pel-played'); play(falling,'en-US'); };
      ...
      ctx.btn.disabled=true;
      ctx.self.ready(ctx, L('Check','تحقق'), ()=>{
        // Version 1 = rising = question, Version 2 = falling = statement
        const correct = sel === 0;
        opts.forEach((b,i)=>{
          b.classList.remove('selected');
        ...
        const why = correct
          ? L('Rising intonation = question. Falling = statement.','النبرة الصاعدة = سؤال. النبرة الهابطة = جملة خبرية.')
          : L('Rising intonation means a question. Falling means a statement.','النبرة الصاعدة تعني سؤال. الهابطة تعني جملة خبرية.');
    ...
    writing_practice(inner, act, ctx){
      ...
      inner.innerHTML = `
        ...
        <div class="pel-act-hint arabic" style="margin-bottom:14px;">${esc(ar)}</div>
        ...
        <div style="margin-bottom:12px;">
          <input type="text" id="wpInput" dir="ltr" placeholder="${L('Type your sentence...','اكتب جملتك...')}"
            style="width:100%;padding:12px;font-size:1.05rem;border:2px solid var(--border);border-radius:10px;background:var(--card);color:var(--text);" />
        </div>
        <div id="wpFb"></div>`;
    ...
    scenario_roleplay(inner, act, ctx){
      ...
      const renderTurn = () => {
        if(turnIdx >= turns.length){
          ...
          ctx.self.ready(ctx, L('Continue','متابعة'), ()=>ctx.self.next());
          if(!done){ done = true; ctx.self.mark(ctx, true); }
          return;
        }
  ...
  const TYPE_LABELS_DEF = {
    ...
    minimal_pairs:['Sounds','اصوات'], intonation:['Intonation','نبرة'],
  ...
  const PROMPTS_DEF = {
    ...
    intonation:['Pick what the intonation means','اختر وش تعني النبرة'],
