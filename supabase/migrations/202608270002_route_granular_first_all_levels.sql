-- ============================================================
-- 202608270002 · Every level starts at its granular skill lessons
-- ------------------------------------------------------------
-- Generalizes 202608270001 (A1 only) to ALL levels (A0..C2, STEP).
-- Before this, student_route() ordered academies by sort_order, so thematic
-- /overview academies with a low sort_order (everyday-english=1,
-- american-conversations=2, writing-workshop=9, a2-complete-course=-100, ...)
-- came BEFORE the granular {level}-* skill academies (sort_order 100-149) and
-- became the starting stage. New A2/B1/B2 students therefore landed on a
-- thematic course instead of their level's first guided skill lesson.
--
-- Fix: an explicit granular-priority map (granular_seq) is joined into the
-- route. Within each level-fit bucket, granular skill academies for the
-- student's level are ordered first (route_priority 0..N), then everything
-- else (route_priority 1000) by sort_order. The map is the curriculum spec:
--   a1 -> a1-core-words, a2 -> a2-past-simple, b1 -> b1-connectors-opinions,
--   b2 -> b2-professional-comm, c1 -> c1-advanced-english,
--   c2 -> c2-near-native, step -> step-exam-prep.
-- Levels with no granular academies (C1/C2/STEP) keep their complete-course
-- start. _prog_for's starting-stage fallback now uses student_route() as the
-- source of truth (first route academy) instead of a sort_order guess, so new
-- students of any level are seated at the granular start.
-- ============================================================
...
begin
  ...
  return coalesce((
    ...
    select jsonb_agg(jsonb_build_object('id', id, 'score', level_fit * 10 + tag_hits)
      order by (level_fit * 10 + tag_hits) desc, route_priority, sort_order, tiebreak)
    from scored
  ), '[]'::jsonb);
end;
$function$;
...
begin
  ...
  if not found then
    ...
    -- starting stage: honor the onboarding goal choice FIRST (active academy),
    -- then best level fit. The chosen path is stored, not guessed per visit.
    begin
